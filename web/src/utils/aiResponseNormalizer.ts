...
export function normalizeNumericField(
  ...
  if (typeof value === "string") {
    ...
    const parsed =
      parseAs === "int" ? parseInt(cleaned, 10) : parseFloat(cleaned);
    return isNaN(parsed) ? defaultValue : parsed;
  }
  return defaultValue;
}
 ...
 */
export function normalizeFlexibleField(
  value: unknown,
...
export function normalizeDialogue(raw: RawAIResponse): NormalizedDialogue {
  ...
  const result: NormalizedDialogue = {
    character_name:
      getString(raw, "character_name", "name") || "Unknown Character",
  ...
  if (!hasValidTree && !result.opening_line) {
    result._parseError =
      "AI response missing expected dialogue structure. Showing available data.";
  }
...
export function safeParseAIResponse<T extends NormalizedResponse>(
  ...
  try {
    ...
    const raw =
      wrapperKey && obj[wrapperKey]
        ? (obj[wrapperKey] as RawAIResponse)
        : (obj as RawAIResponse);
    ...
    if (normalized._raw && Object.keys(normalized._raw).length > 0) {
      warnings.push(
        `Response contained ${Object.keys(normalized._raw).length} unexpected fields`,
      );
    }
