...
function parseStats(stats: string | MonsterStats | undefined): MonsterStats | null {
...
export function MonsterDetail({ monster }: MonsterDetailProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {stats && (
        <div>
          <h3 className="text-lg font-semibold text-primary mb-3">Combat Statistics</h3>
          <div className="bg-surface p-6 rounded-lg border border-border space-y-4">
            {stats.armor_class && (
              <div className="flex justify-between">
                ...
                <span className="text-text font-semibold">{stats.armor_class}</span>
              </div>
            )}
            {stats.hit_points && (
              <div className="flex justify-between">
                <span className="text-text-muted">Hit Points</span>
                <span className="text-text font-semibold">{formatHitPoints(stats.hit_points)}</span>
              </div>
            )}
            {stats.speed && (
              <div>
      ...
      {stats?.abilities && (
        <div>
          <h3 className="text-lg font-semibold text-primary mb-3">Ability Scores</h3>
      ...
      {monster.lore && (
        <div>
          ...
          <p className="text-text leading-relaxed whitespace-pre-wrap">{monster.lore}</p>
        </div>
      )}
      ...
      {monster.tactics && (
        <div>
          ...
          <p className="text-text leading-relaxed whitespace-pre-wrap">{monster.tactics}</p>
        </div>
      )}
          ...
          <div className="space-y-3">
            {stats.actions.map((action, idx) => (
              <div key={idx} className="bg-surface p-4 rounded-lg border border-border">
                <div className="font-semibold text-text mb-1">{action.name}</div>
