    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="flex gap-2 flex-wrap mb-3">
          ...
          {item.rarity && (
            <span className={`px-3 py-1 rounded-lg text-sm capitalize ${getRarityClasses(item.rarity)}`}>
      ...
      {properties && (
        <div>
          <h3 className="text-lg font-semibold text-primary mb-3">Properties</h3>
          <div className="bg-surface p-4 rounded-lg border border-border space-y-2">
            {Object.entries(properties).map(([key, val]) => (
              <div key={key} className="flex justify-between">
                ...
                <span className="text-text">{formatPropertyValue(key, val)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {item.origin && (
        <div>
          ...
          <p className="text-text leading-relaxed whitespace-pre-wrap">{item.origin}</p>
        </div>
      )}
      ...
      {item.curse && (
        <div>
          ...
          <div className="bg-red-900/20 p-4 rounded-lg border border-red-500/30">
            <p className="text-text leading-relaxed whitespace-pre-wrap">{item.curse}</p>
          </div>
        </div>
      )}
        ...
        <div className="bg-primary/10 p-4 rounded-lg border border-primary/30">
          <div className="flex gap-6">
            {(value || item.value !== undefined) && (
              <div>
                <div className="text-xs text-text-muted uppercase tracking-wide mb-1">Value</div>
            ...
            {item.weight !== undefined && (
              <div>
                <div className="text-xs text-text-muted uppercase tracking-wide mb-1">Weight</div>
                <div className="text-text text-lg">
                  {typeof item.weight === 'object' && (item.weight as ItemWeight).amount
