  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        {tavern.quality && (
          <div className="bg-surface p-4 rounded-lg border border-border">
            <div className="text-xs text-text-muted uppercase tracking-wide mb-2">Quality</div>
            <div className="text-lg text-text capitalize">{tavern.quality}</div>
          </div>
        )}
        {tavern.size && (
          <div className="bg-surface p-4 rounded-lg border border-border">
            <div className="text-xs text-text-muted uppercase tracking-wide mb-2">Size</div>
      ...
      {tavern.atmosphere && (
        <div>
          <h3 className="text-lg font-semibold text-primary mb-2">Atmosphere</h3>
          <p className="text-text leading-relaxed">{tavern.atmosphere}</p>
          {tavern.description && (
            <p className="text-text leading-relaxed mt-2">{tavern.description}</p>
          )}
        </div>
      )}
      ...
      {(tavern.keeper_name || tavern.keeper_personality || tavern.keeper_description) && (
        <div>
          <h3 className="text-lg font-semibold text-primary mb-2">The Keeper</h3>
          <div className="bg-surface p-4 rounded-lg border border-border space-y-2">
            {tavern.keeper_name && (
              <h4 className="font-semibold text-text text-lg">{tavern.keeper_name}</h4>
            )}
            {tavern.keeper_personality && (
              <p className="text-text italic text-sm">{tavern.keeper_personality}</p>
            )}
        ...
        <div>
          ...
          <div className="bg-surface p-4 rounded-lg border border-border space-y-4">
            {Array.isArray(menuFood) && menuFood.length > 0 && (
              <div>
                <h4 className="text-sm font-semibold text-text-muted mb-2">Food</h4>
                <div className="space-y-2">
                  {menuFood.map((item, idx) => (
                    <div key={idx} className="flex justify-between items-start">
                      ...
                      <span className="text-primary ml-4 whitespace-nowrap">{item.price}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {Array.isArray(menuDrinks) && menuDrinks.length > 0 && (
              <div>
                <h4 className="text-sm font-semibold text-text-muted mb-2">Drinks</h4>
                <div className="space-y-2">
      ...
      {Array.isArray(rooms) && rooms.length > 0 && (
        <div>
          <h3 className="text-lg font-semibold text-primary mb-3">Accommodations</h3>
              ...
              <div key={idx} className="bg-surface p-4 rounded-lg border border-border">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    ...
                    {room.available && (
                      <span className="text-text-muted text-xs">({room.available} available)</span>
                    )}
                  </div>
                  <span className="text-primary whitespace-nowrap">{room.price}</span>
                </div>
                <p className="text-text text-sm">{room.description || room.details}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {Array.isArray(patrons) && patrons.length > 0 && (
        <div>
          <h3 className="text-lg font-semibold text-primary mb-3">Notable Patrons</h3>
          <div className="grid gap-3">
      ...
      {Array.isArray(events) && events.length > 0 && (
        <div>
          <h3 className="text-lg font-semibold text-primary mb-3">Current Events</h3>
      ...
      {Array.isArray(rumors) && rumors.length > 0 && (
        <div>
          <h3 className="text-lg font-semibold text-primary mb-3">Rumors & Gossip</h3>
          <ul className="list-disc list-inside space-y-1">
