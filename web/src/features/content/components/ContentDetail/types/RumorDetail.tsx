  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-surface p-6 rounded-lg border border-border">
        <p className="text-text text-xl italic leading-relaxed">&ldquo;{rumor.text}&rdquo;</p>
      </div>

      {/* Veracity */}
      <div className="bg-surface p-4 rounded-lg border border-border inline-block">
        <div className="text-xs text-text-muted uppercase tracking-wide mb-2">Veracity</div>
        <span className={`px-3 py-1 rounded text-sm inline-block ${getVeracityClasses(rumor.veracity)}`}>
      ...
      {rumor.context && (
        <div>
          ...
          <p className="text-text leading-relaxed whitespace-pre-wrap">{rumor.context}</p>
        </div>
      )}
      ...
      {rumor.foreshadowing && (
        <div className="bg-primary/10 p-4 rounded-lg border border-primary/30">
          <div className="flex items-center gap-2">
            ...
            <span className="text-primary font-semibold">Foreshadows Future Events</span>
          </div>
        </div>
      )}
      ...
      {tags.length > 0 && (
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {tags.map((tag, idx) => (
              <span key={idx} className="px-3 py-1 bg-surface text-text rounded-full text-sm">
                {tag}
              </span>
            ))}
          </div>
