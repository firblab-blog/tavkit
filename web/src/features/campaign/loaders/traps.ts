...
export async function loadTraps(campaignId: string): Promise<CampaignContent[]> {
  ...
  return traps.map((trap: any) => {
    ...
    if (detection && (detection.passive_perception_dc || detection.investigation_dc)) {
      detectionDisplay = `\n\n**Detection:**\n`
    ...
    if (Array.isArray(complications)) {
      complicationsDisplay = complications.map((comp) => `- ${comp}`).join('\n')
    }
