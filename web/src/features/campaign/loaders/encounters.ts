...
export async function loadEncounters(
  ...
  const encounters = await fetchContentData<any>(
    "encounters",
    campaignId,
    "encounters",
  );
  ...
  return encounters.map((encounter: any) => {
    ...
    return {
      ...
      content: [
        ...
        encounter.xp_total !== undefined
          ? `**XP Total:** ${encounter.xp_total}  `
          : "",
        encounter.xp_per_player !== undefined
          ? `**XP per Player:** ${encounter.xp_per_player}  `
          : "",
        "",
