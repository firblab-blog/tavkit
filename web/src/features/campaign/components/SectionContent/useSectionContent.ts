...
export function useSectionContent({
  ...
  }, [campaignId, sectionId, subsection, effectiveSection, isDedicatedType, fetchCampaignContent, onEntriesLoad])
  ...
  const handleSaveEntry = useCallback(async () => {
    ...
    try {
      if (isDedicatedType) {
        ...
        const body = buildCreateBody(effectiveSection, title, content, campaignId)
        ...
        if (editingEntry) {
          await updateCampaignContent(campaignId, editingEntry.id, { title, content })
        } else {
  ...
  const handleFileUpload = useCallback(
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        if (isDedicatedType) {
          const endpoint = getApiUrl(`/${effectiveSection}`)
          const body = buildCreateBody(effectiveSection, fileName, fileContent, campaignId)
...
function buildCreateBody(
  ...
  switch (type) {
    case 'npcs':
      return { ...base, race: '', class: '', personality: '', backstory: content }
    case 'items':
      return { ...base, description: content, type: 'Other', rarity: 'Common' }
    case 'monsters':
      return { ...base, lore: content, cr: 1, stats: {} }
    case 'encounters':
      return { ...base, description: content, difficulty: 'Medium', party_level: 1, party_size: 4, creatures: [] }
    case 'dialogues':
      return { ...base, character_name: title, scene_setting: '', mood: '', dialogue_tree: [{ text: content }] }
    case 'rumors':
      return { ...base, text: content, source: '', veracity: 'true' }
    case 'locations':
      return { ...base, description: content, type: 'Other' }
    case 'quests':
      return { ...base, title, type: 'side', description: content, status: 'available' }
    case 'taverns':
      return { ...base, atmosphere: content }
    case 'merchants':
      return { ...base, shop_type: 'general', atmosphere: content }
    case 'traps':
      return { ...base, description: content, trap_type: 'mechanical', difficulty: 'medium' }
    case 'critters':
      return { ...base, description: content, critter_type: 'beast', size: 'medium' }
    case 'chases':
      return { ...base, description: content, chase_type: 'pursuit', terrain: 'urban', difficulty: 'medium' }
    default:
      return base
  }
}
