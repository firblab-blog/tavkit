...
} from "../renderers/EncounterRenderer";
import {
...
export function EncounterGenerator() {
  ...
  const encounterConfig = useMemo(
    ...
    > => ({
      ...
      saveApi: (data: unknown) =>
        saveEncounter(data as Record<string, unknown>),
      normalizeResponse: (raw: Record<string, unknown>) => {
        ...
        if (raw.encounter && typeof raw.encounter === "object") {
          const normalized = normalizeEncounterResponse(
            raw.encounter as Record<string, unknown>,
          );
      ...
      buildSavePayload: (
        ...
        return {
          ...
          party_level:
            typeof currentForm.party_level === "number"
              ? currentForm.party_level
              : 5,
          party_size:
            typeof currentForm.party_size === "number"
              ? currentForm.party_size
              : 4,
          difficulty:
            encounter.difficulty || currentForm.difficulty || "medium",
  ...
  const handleGenerate = useCallback(() => {
    state.generate({
      ...
      party_size:
        typeof formData.party_size === "number" ? formData.party_size : 4,
      difficulty: formData.difficulty || "medium",
      environment:
        formData.environment !== "random" ? formData.environment : "random",
  ...
  const handleCopy = useCallback(() => {
    if (state.generatedData) {
      navigator.clipboard.writeText(
        formatEncounterForClipboard(state.generatedData),
      );
    }
  }, [state.generatedData]);
  ...
  return (
    <>
      <GeneratorLayout
        ...
        icon="Swords"
        formTitle={
          state.entryMode === "ai" ? "Encounter Parameters" : "Manual Entry"
        }
        formIcon={state.entryMode === "ai" ? "Settings" : "Edit"}
        resultsTitle={
          state.entryMode === "ai" ? "Generated Encounter" : "Preview"
        }
