...
import { CritterRenderer, formatCritterForClipboard } from '../renderers/CritterRenderer'
import {
...
import { generateCritter, saveCritter, type CritterGenerationRequest } from '@/api/generators'
...
const critterConfig: GeneratorConfig<GeneratedCritterData, ManualCritterData, CritterParams> = {
...
export function CritterGenerator() {
  ...
  const handleCopy = useCallback(() => {
    if (state.generatedData) {
      navigator.clipboard.writeText(formatCritterForClipboard(state.generatedData))
    }
  }, [state.generatedData])
  ...
  return (
    <>
      <GeneratorLayout
        ...
        icon="PawPrint"
        formTitle={state.entryMode === 'ai' ? 'Critter Details' : 'Manual Entry'}
        formIcon={state.entryMode === 'ai' ? 'Settings' : 'Edit'}
        resultsTitle={state.entryMode === 'ai' ? 'Generated Critter' : 'Preview'}
