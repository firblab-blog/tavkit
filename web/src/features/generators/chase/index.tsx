...
import { ChaseRenderer, formatChaseForClipboard } from '../renderers/ChaseRenderer'
import {
...
export function ChaseGenerator() {
  ...
  const chaseConfig = useMemo((): GeneratorConfig<
    GeneratedChaseData,
    ManualChaseData,
  ...
  > => ({
    ...
    normalizeResponse: (raw: Record<string, unknown>) => {
      ...
      if (raw.chase && typeof raw.chase === 'object') {
        const normalized = normalizeChaseResponse(raw.chase as Record<string, unknown>)
    ...
    buildSavePayload: (chase: GeneratedChaseData, campaignId: string | null) => {
    ...
    buildManualSavePayload: (data: ManualChaseData, campaignId: string | null) => ({
      campaign_id: campaignId || undefined,
      name: data.name.trim(),
      ...
      environmental_factors: data.environmental_factors.filter((e: string) => e.trim()),
      ai_generated: false,
      starting_distance: 3,
      catch_threshold: 0,
      escape_threshold: 7,
  ...
  const handleGenerate = useCallback(() => {
    state.generate({
      ...
      party_level: String(typeof formData.party_level === 'number' ? formData.party_level : 5),
  ...
  const handleCopy = useCallback(() => {
    if (state.generatedData) {
      navigator.clipboard.writeText(formatChaseForClipboard(state.generatedData))
    }
  }, [state.generatedData])
