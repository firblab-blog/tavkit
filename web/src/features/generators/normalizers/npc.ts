...
function extractCombatStats(data: Record<string, unknown>): NPCCombatStats | undefined {
...
const EXPECTED_FIELDS = [
  ...
  'personality_traits', 'traits', 'character_traits', 'background', 'motivation',
  'abilities', 'stats', 'ability_scores', 'attributes', 'combat', 'skills',
  'equipment', 'role', 'plot_hooks', 'provider', '_parse_warning', '_raw',
  'str', 'dex', 'con', 'int', 'wis', 'cha', 'strength', 'dexterity',
  'constitution', 'intelligence', 'wisdom', 'charisma', 'hp', 'ac', 'speed',
  'hit_points', 'armor_class',
]

export function normalizeNPCResponse(raw: Record<string, unknown>): GeneratedNPCData {
  ...
  if (raw.background && typeof raw.background === 'string') {
    ...
    if (bgStr.startsWith('{') && bgStr.endsWith('}')) {
      try {
        ...
        logger.debug('[normalizeNPC] Parsed NPC from JSON background:', parsedNPC)
