...
export function normalizeMerchantResponse(raw: Record<string, unknown>): GeneratedMerchantData {
  ...
  if (raw.description && typeof raw.description === 'string') {
    ...
    if (descStr.startsWith('{') && descStr.endsWith('}')) {
      try {
        ...
        logger.debug('[normalizeMerchant] Parsed merchant from JSON description:', parsedMerchant)
        processedRaw = parsedMerchant
      } catch (e) {
        logger.warn('[normalizeMerchant] Failed to parse description as JSON:', e)
      }
    }
  }
...
export function hasValidMerchantContent(merchant: GeneratedMerchantData): boolean {
  return !!(
    merchant.name &&
