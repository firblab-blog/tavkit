...
export function getNumericValue(value: number | ValueObject | WeightObject): number {
...
export function normalizeItemResponse(raw: Record<string, unknown>): GeneratedItemData {
  ...
  if (raw.description && typeof raw.description === 'string') {
    ...
    if (descStr.startsWith('{') && descStr.endsWith('}')) {
      try {
        ...
        logger.debug('[normalizeItem] Parsed item from JSON description:', parsedItem)
