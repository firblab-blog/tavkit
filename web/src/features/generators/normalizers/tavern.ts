...
export function normalizeTavernResponse(raw: Record<string, unknown>): GeneratedTavernData {
  ...
  if (raw.description && typeof raw.description === 'string') {
    ...
    if (descStr.startsWith('{') && descStr.endsWith('}')) {
      try {
        ...
        logger.debug('[normalizeTavern] Parsed tavern from JSON description:', parsedTavern)
        processedRaw = parsedTavern
      } catch (e) {
        logger.warn('[normalizeTavern] Failed to parse description as JSON:', e)
      }
    }
  }
