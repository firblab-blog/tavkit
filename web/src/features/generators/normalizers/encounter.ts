 ...
 */
export function hasValidEncounterContent(encounter: GeneratedEncounterData | null): boolean {
 ...
 */
export function normalizeEncounterResponse(raw: Record<string, unknown>): GeneratedEncounterData {
  ...
  if (raw.description && typeof raw.description === 'string') {
    ...
    if (descStr.startsWith('{') && descStr.endsWith('}')) {
      try {
        ...
        logger.debug('[EncounterNormalizer] Parsed encounter from JSON description:', parsedEncounter)
        processedRaw = parsedEncounter
      } catch (e) {
        logger.warn('[EncounterNormalizer] Failed to parse description as JSON:', e)
      }
    }
  }
