...
export interface ChaseComplication {
  ...
  round: number
  description: string
  complication_type: 'obstacle' | 'hazard' | 'bystander' | 'terrain_change' | 'reinforcement'
  effect?: string
...
export function getParticipantRoleLabel(role: ChaseParticipant['role']): string {
  return role === 'pursuer' ? 'Pursuer' : 'Quarry'
}

export function getComplicationTypeLabel(type: ChaseComplication['complication_type']): string {
...
export function getDCFromDifficulty(difficulty: string): { min: number; max: number } {
