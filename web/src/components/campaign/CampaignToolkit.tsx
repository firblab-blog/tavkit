...
logger.debug('[CampaignToolkit] Module loaded - BUILD TIMESTAMP:', new Date().toISOString())
...
export default function CampaignToolkit() {
  ...
  const [campaignSections, setCampaignSections] = useState<CampaignSection[]>(visibleSections)
  ...
  useEffect(() => {
    ...
    if (!active) {
      const filtered = defaultCampaignSections.filter((s) => !hiddenSections.includes(s.id))
    ...
    try {
      if (active.setting) {
        const settingObj =
          typeof active.setting === 'string' ? JSON.parse(active.setting) : active.setting
        // Sections order
        if (settingObj && Array.isArray(settingObj.sections_order)) {
          ...
          const missing = defaultCampaignSections.filter((s) => !order.includes(s.id))
          ...
          if (missing.some((s) => s.id === 'artificers-toolkit')) {
            ...
            const artificersToolkit = missing.find((s) => s.id === 'artificers-toolkit')!
            const otherMissing = missing.filter((s) => s.id !== 'artificers-toolkit')
          ...
          if (settingObj.subsections_order && typeof settingObj.subsections_order === 'object') {
            sectionsWithSubsections = sectionsWithSubsections.map((sec) => {
              const key = sec.id
              const subsOrder = settingObj.subsections_order[key]
              if (Array.isArray(subsOrder) && sec.subsections && sec.subsections.length > 0) {
                const orderedSubs = subsOrder
                  .map((name: string) => sec.subsections!.find((s) => s === name))
                  .filter(Boolean) as string[]
                const missingSubs = sec.subsections!.filter((s) => !subsOrder.includes(s))
                return { ...sec, subsections: [...orderedSubs, ...missingSubs] }
              }
              return sec
            })
          }
          ...
          const filtered = sectionsWithSubsections.filter((s) => !hiddenSections.includes(s.id))
          setCampaignSections(filtered)
          return
        }
      }
    } catch (e) {
      logger.warn('Failed to parse campaign setting for sections_order/subsections_order', e)
    }
    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          logger.error('Failed to fetch campaign context:', response.status, response.statusText)
        }
      } catch (fetchErr) {
        if (timeoutId) clearTimeout(timeoutId)
      ...
      <div className="flex-shrink-0 border-b border-border bg-background-panel px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            {isMobile && activeCampaignId && campaigns.find((c) => c.id === activeCampaignId) && (
              <button
            ...
            <div>
              <h1 className="text-2xl font-bold text-text flex items-center gap-2">
                ...
                {activeCampaign ? activeCampaign.name || 'Untitled Campaign' : 'Campaign Ledger'}
              </h1>
              ...
              <nav className="space-y-6">
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    ...
                    {campaignSections
                      .filter(
                        (section) => section.id === 'summary' || section.id === 'artificers-toolkit'
                      )
                      .map((section) => {
                            ...
                            {section.subsections &&
                              ...
                              isExpanded && (
                                <div className="ml-6 mt-1 space-y-1">
                                  {filterSubsections(section.subsections).map((sub) => {
                                    const subsectionLabels: Record<
                                      string,
                                      { name: string; icon: IconName }
                                    > = {
                                      npcs: { name: 'NPCs', icon: 'Users' },
                                      monsters: { name: 'Monsters', icon: 'Skull' },
                                      encounters: { name: 'Encounters', icon: 'Swords' },
                                      dialogues: { name: 'Dialogues', icon: 'MessageCircle' },
                                      locations: { name: 'Locations', icon: 'Map' },
                                      quests: { name: 'Quests', icon: 'Scroll' },
                                      items: { name: 'Items', icon: 'Package' },
                                      rumors: { name: 'Rumors', icon: 'MessageSquare' },
                                      taverns: { name: 'Taverns', icon: 'Beer' },
                                      merchants: { name: 'Merchants', icon: 'Package' },
                                      traps: { name: 'Traps', icon: 'AlertCircle' },
                                      critters: { name: 'Critters', icon: 'Shield' },
                                      chases: { name: 'Chases', icon: 'Sparkles' },
                                    }
                                    const label = subsectionLabels[sub] || {
                                      name: sub,
                                      icon: 'Sparkles' as IconName,
                                    }
                                    const isSubsectionSelected = selectedSection === sub
                                    ...
                                    return (
                                      ...
                                      >
                                        <Icon name={label.icon} className="w-3 h-3" />
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    {campaignSections
                      .filter((section) =>
                        ['overview', 'sessions', 'pcs', 'factions', 'lore'].includes(section.id)
                      )
                      .map((section) => {
                        const isCurrentSection = selectedSection === section.id
                        const sectionHasEntries = isCurrentSection && sectionEntries.length > 0
                        const sectionHasSubsections =
                          section.subsections && section.subsections.length > 0
                        const isExpanded = expandedSections.has(section.id)
                        ...
                        return (
                          <div key={section.id}>
                            <button
                              ...
                              className={`w-full text-left px-3 py-2 rounded-lg transition-colors flex items-center gap-2 ${
                                isCurrentSection && !selectedEntryId && !sectionHasSubsections
                                  ? 'bg-primary text-tavern-darkest'
                                  : 'text-tavern-mauve hover:bg-tavern-dark hover:text-tavern-light'
                              }`}
                            >
                            ...
                            {/* Entry List */}
                            {isCurrentSection && sectionHasEntries && isExpanded && (
                              <div className="ml-6 mt-1 space-y-1">
                              ...
                              isExpanded && (
                                <div className="ml-6 mt-1 space-y-1">
                                  {filterSubsections(section.subsections).map((sub) => {
                                    ...
                                    const subsectionInfo = subsectionLabels[sub] || {
                                      name: sub,
                                    ...
                                    return (
                                      ...
                                      >
                                        <Icon name={subsectionInfo.icon} className="w-3 h-3" />
                                        {subsectionInfo.name}
                                      </button>
                                    )
                                  })}
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    {campaignSections
                      .filter((section) =>
                        ['maps', 'handouts', 'props', 'art', 'soundscapes'].includes(section.id)
                      )
                      .map((section) => {
                        const isCurrentSection = selectedSection === section.id
                        const sectionHasSubsections =
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    {campaignSections
                      .filter((section) =>
                        ['statblocks', 'gm-notes', 'tracking'].includes(section.id)
                      )
                      .map((section) => {
                        const isCurrentSection = selectedSection === section.id
                        const sectionHasSubsections =
              ...
              {selectedSection === 'summary' ? (
                ...
                <SectionContent
                  ...
                  section={(() => {
                    ...
                    const topLevel = campaignSections.find((s) => s.id === selectedSection)
                    ...
                    > = {
                      ...
                      taverns: {
                        name: 'Taverns',
                        icon: 'Beer',
                        description: 'Inns, taverns, and drinking establishments',
                      },
                      merchants: {
                        name: 'Merchants',
                        icon: 'Package',
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="p-6">
            <div className="max-w-6xl mx-auto">
              ...
              {loading && (
                <div className="text-center py-12 text-tavern-mauve">Loading campaigns...</div>
              )}
              {error && (
              ...
              {!loading && lastFetchTime && campaigns.length === 0 ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  <Icon name="BookMarked" className="w-20 h-20 text-tavern-mauve mb-4" />
                  <h3 className="text-2xl font-bold text-tavern-light mb-2">No Campaigns Yet</h3>
                  <p className="text-tavern-mauve max-w-md mb-6">
                    Create your first campaign to start organizing NPCs, locations, quests, and
                    more.
                  </p>
                  <button
                    onClick={handleCreateCampaign}
                      ...
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <div className="flex gap-1">
                          <button
                            ...
                            title="Edit"
                          >
                            <Icon name="Edit" className="w-4 h-4 text-tavern-mauve" />
                          </button>
                          <button
      ...
      {sectionContextMenu && (
        ...
        >
          <button
            onClick={async () => {
              ...
              if (sectionContextMenu.kind === 'section') {
                const idx = campaignSections.findIndex((s) => s.id === sectionContextMenu.id)
                if (idx > 0) {
                  ...
                  if (active) {
                    ...
                    try {
                      await updateCampaign(active.id, { ...active, setting: updatedSetting })
                    } catch (e) {
                ...
                // Entry move up
                const idx = sectionEntries.findIndex((e) => e.id === sectionContextMenu.id)
                if (idx > 0) {
                  ...
                  if (active) {
                    const currentSetting =
                      active.setting && typeof active.setting === 'string'
                        ? JSON.parse(active.setting)
                        : active.setting || {}
                    const entries_order = { ...(currentSetting.entries_order || {}) }
                    entries_order[sectionContextMenu.parentId!] = newEntries.map((e) => e.id)
                ...
                const parentIdx = campaignSections.findIndex((s) => s.id === parentId)
                if (parentIdx >= 0) {
                  const parent = campaignSections[parentIdx]
                  const subs = parent.subsections ? [...parent.subsections] : []
                  const subIdx = subs.findIndex((x) => x === sectionContextMenu.id)
                  if (subIdx > 0) {
                    ...
                    if (active) {
                      const currentSetting =
                        active.setting && typeof active.setting === 'string'
                          ? JSON.parse(active.setting)
                          : active.setting || {}
                      const subsections_order = { ...(currentSetting.subsections_order || {}) }
                      subsections_order[parentId] = subs
                      const updatedSetting = { ...currentSetting, subsections_order }
                      try {
                        await updateCampaign(active.id, { ...active, setting: updatedSetting })
            ...
            disabled={
              sectionContextMenu.kind === 'section'
                ? campaignSections.findIndex((s) => s.id === sectionContextMenu.id) === 0
                : sectionContextMenu.kind === 'entry'
                  ...
                  : sectionContextMenu.kind === 'subsection'
                    ? (() => {
                        const parentId = sectionContextMenu.parentId!
                        const parent = campaignSections.find((s) => s.id === parentId)
                        if (!parent || !parent.subsections) return true
                        return (
                          parent.subsections.findIndex((x) => x === sectionContextMenu.id) === 0
                        )
                      })()
                    : true
            }
          ...
          <button
            ...
            disabled={
              sectionContextMenu.kind === 'section'
                ? campaignSections.findIndex((s) => s.id === sectionContextMenu.id) ===
                ...
                : sectionContextMenu.kind === 'entry'
                  ...
                  : sectionContextMenu.kind === 'subsection'
                    ? (() => {
                        ...
                        if (!parent || !parent.subsections) return true
                        return (
                          parent.subsections.findIndex((x) => x === sectionContextMenu.id) ===
