...
export default function PartyLoot() {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (window.confirm('Are you sure you want to delete this item from party loot?')) {
  ...
  if (!activeCampaign) {
    return (
      <div className="text-center py-12 bg-background-panel border border-border rounded-xl">
        <Icon name="Package" className="w-12 h-12 text-text-muted mx-auto mb-4" />
        <h3 className="text-lg font-medium text-text mb-2">No Active Campaign</h3>
        <p className="text-text-muted">Select or join a campaign to track party loot.</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <select
          value={filterClaimed}
          onChange={(e) => setFilterClaimed(e.target.value as 'all' | 'unclaimed' | 'claimed')}
      ...
      {!loading && sortedItems.length === 0 && (
        <div className="text-center py-12 bg-background-panel border border-border rounded-xl">
          ...
          <h3 className="text-lg font-medium text-text mb-2">
            {searchQuery || filterClaimed !== 'all' ? 'No matching items' : 'No loot yet'}
          </h3>
      ...
      {showEditor && activeCampaign && (
        <LootEditor item={editingItem} campaignId={activeCampaign.id} onClose={handleEditorClose} />
      )}
