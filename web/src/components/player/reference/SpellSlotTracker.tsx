...
export default function SpellSlotTracker({
  ...
  useEffect(() => {
    setCustomSlots(
      spellSlots.length > 0 ? spellSlots : [{ level: 1, max: 2, used: 0 }],
    );
  }, [spellSlots]);
  ...
  const addSlotLevel = () => {
    const nextLevel =
      customSlots.length > 0
        ? Math.max(...customSlots.map((s) => s.level)) + 1
        : 1;
    if (nextLevel <= 9) {
  ...
  const updateSlotMax = (level: number, max: number) => {
    setCustomSlots(
      customSlots.map((s) =>
        s.level === level ? { ...s, max: Math.max(0, max) } : s,
      ),
    );
  };
      ...
      <div className="bg-background-panel border border-border rounded-xl p-4">
        ...
        <div className="border-t border-border pt-4">
          <p className="text-sm text-text-muted mb-3">
            Or customize your slots:
          </p>
          <div className="space-y-2 mb-4">
            {customSlots.map((slot) => (
              <div key={slot.level} className="flex items-center gap-3">
                <span className="text-text w-20">
                  {getOrdinal(slot.level)} Level
                </span>
                <input
                  type="number"
                  value={slot.max}
                  onChange={(e) =>
                    updateSlotMax(slot.level, parseInt(e.target.value) || 0)
                  }
                  min="0"
                  max="10"
      ...
      <div className="space-y-3">
        {spellSlots.map((slot) => {
          ...
          return (
            <div key={slot.level} className="flex items-center gap-3">
              {/* Level label */}
              <div className="w-16 text-sm font-medium text-text">
                {getOrdinal(slot.level)}
              </div>
                ...
                {Array.from({ length: slot.max }).map((_, idx) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() =>
                        isUsed
                          ? handleRestoreSlot(slot.level)
                          : handleUseSlot(slot.level)
                      }
                      disabled={loading}
                    ...
                    >
                      {!isUsed && (
                        <Icon
                          name="Sparkles"
                          className="w-4 h-4 text-purple-400 mx-auto"
                        />
                      )}
                    </button>
                  );
              ...
              <div className="w-12 text-right text-sm">
                <span
                  className={
                    available > 0 ? "text-purple-400" : "text-gray-500"
                  }
                >
                  {available}
                </span>
