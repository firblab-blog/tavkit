...
export default function PlayerCombatView({
  ...
  const fetchGMCombat = useCallback(async () => {
    ...
    try {
      const response = await apiClient.get(
        `/campaigns/${activeCampaignId}/combat/active`,
      );
      ...
      if (activeCombat && activeCharacterId) {
        ...
        const currentParticipant =
          sortedParticipants[activeCombat.current_turn];
    ...
    (data: { type: string; payload: unknown }) => {
      switch (data.type) {
        case "combat:state": {
          ...
          const sorted = [...payload.participants].sort(
        ...
        case "combat:turn_changed": {
          ...
          setGMCombat((prev) => {
            ...
            const myP = prev.participants.find(
              (p) => p.character_id === activeCharacterId,
            );
  ...
  useEffect(() => {
    if (activeCharacter && combat.max_hp > 0) {
      ...
      if (totalHP !== combat.max_hp && totalHP > 0) {
        ...
        const currentHP =
          combat.current_hp === activeCharacter.max_hp
            ? totalHP
            : combat.current_hp;
        setLocalCombat({
  ...
  const handleEndCombat = async () => {
    ...
    if (
      window.confirm(
        "End combat? This will clear your initiative and concentration.",
      )
    ) {
  ...
  const isInGMCombat = gmCombat.participants.some(
    (p) => p.character_id === activeCharacterId,
  );
    ...
    return (
      <div className="text-center py-12 bg-background-panel border border-border rounded-xl">
        ...
        <p className="text-text-muted">
          Select a character to use the combat assistant.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-4" id="player-combat">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-text-muted mb-3">
                {gmCombat.combat?.name} • Round{" "}
                {(gmCombat.combat?.current_round ?? 0) + 1}
              </p>
              <button
      ...
      {gmCombat.combat && (
        ...
        >
            ...
            {gmCombat.participants.map((participant, index) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`flex-1 font-medium ${isMe ? "text-blue-400" : "text-text"}`}
                  >
                    {participant.name}
                    {isMe && (
          ...
          {!gmCombat.participants.some(
            (p) => p.character_id === activeCharacterId,
          ) && (
            <div className="mt-4 p-3 bg-amber-500/10 border border-amber-500/30 rounded-lg">
              <p className="text-amber-300 text-sm">
                <Icon name="Info" className="w-4 h-4 inline mr-2" />
                Your character hasn&apos;t joined this combat yet. Enter your
                initiative above to join.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="bg-background-panel border border-border rounded-xl p-4">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between p-3 bg-background rounded-lg">
              ...
              <button
                onClick={() =>
                  activeCharacterId && toggleReaction(activeCharacterId)
                }
              ...
              return (
                ...
                >
                  ...
                  <button
                    onClick={() =>
                      activeCharacterId &&
                      removeCondition(activeCharacterId, condition.type)
                    }
