      ...
      <div className="bg-background-panel border border-border rounded-xl max-w-md w-full shadow-2xl">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-3">
            <label className="text-sm font-medium text-text">Roll Initiative</label>
          ...
          {mode === 'auto' ? (
                ...
                <div className="bg-background rounded-lg p-4 border border-primary/30">
                  <div className="flex items-center justify-between">
                    ...
                    <div>
                      ...
                      <p className="text-2xl font-bold text-primary">+{initiativeBonus}</p>
                    </div>
                    <div className="text-2xl text-text-muted">=</div>
                    <div>
                      <p className="text-sm text-text-muted">Total</p>
                      <p className="text-3xl font-bold text-primary">{totalInitiative}</p>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="space-y-3">
              <label className="text-sm text-text-muted">Enter your d20 roll (1-20)</label>
              <input
                type="number"
                min="1"
                max="20"
              ...
              {manualRoll && !Number.isNaN(Number.parseInt(manualRoll)) && (
                <div className="bg-background rounded-lg p-4 border border-primary/30">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm text-text-muted">d20 Roll</p>
                      <p className="text-2xl font-bold text-text">{Number.parseInt(manualRoll)}</p>
                    </div>
                    <div className="text-2xl text-text-muted">+</div>
                    <div>
                      <p className="text-sm text-text-muted">Bonus</p>
        ...
        <div className="border-t border-border px-6 py-4 flex items-center justify-end gap-3">
          ...
          <button
            ...
            disabled={
              ...
              (mode === 'manual' && (!manualRoll || Number.isNaN(Number.parseInt(manualRoll))))
            }
