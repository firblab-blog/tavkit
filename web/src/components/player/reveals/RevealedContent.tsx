...
export default function RevealedContent() {
  const { playerReveals, loading, error, fetchPlayerReveals } =
    useContentRevealsStore();
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-text-muted mt-1">
            Content your GM has revealed to the party.
          </p>
        </div>
        <select
          value={filterType}
                  ...
                  >
                    <div className="p-4">
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                              <span
                                className={`px-1.5 py-0.5 text-xs rounded ${levelBadge.color}`}
                              >
                                {levelBadge.text}
                              </span>
                        ...
                        {(reveal.reveal_level !== "name_only" ||
                          reveal.custom_notes) && (
                          <button
                            onClick={() =>
                              setExpandedId(isExpanded ? null : reveal.id)
                            }
                      ...
                      {reveal.custom_notes && (
                        <div className="mt-2 p-2 bg-purple-500/10 rounded-lg border border-purple-500/20">
                          <p className="text-purple-200 text-sm">
                            <span className="font-medium">GM Note:</span>{" "}
                            {reveal.custom_notes}
                          </p>
                        </div>
                      )}
