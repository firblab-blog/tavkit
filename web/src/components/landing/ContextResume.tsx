...
export default function ContextResume() {
  ...
  const lastCampaign = campaigns.find(
    (c) => c.id === userContext?.last_campaign_id,
  );
  ...
  const visibleCampaigns = showAllCampaigns
    ? allCampaigns
    : allCampaigns.slice(0, MAX_VISIBLE);
    ...
    <div className="h-full flex flex-col items-center justify-center px-4 py-8 bg-background relative overflow-auto">
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="text-center mb-6">
          ...
          {honorific && (
            <p className="text-sm text-primary italic">{honorific}</p>
          )}
        </div>
