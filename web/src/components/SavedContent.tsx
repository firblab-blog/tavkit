...
export default function SavedContent() {
  ...
  const fetchContent = async (type: ContentType) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || err.message || `Failed to load ${type}`)
    } finally {
      setLoading(false)
    }
  }
            ...
            <div className="p-4">
              ...
              <nav className="space-y-1">
                {tabs.map((tab) => (
                  ...
                  >
                    <Icon name={tab.iconName} className="w-5 h-5 flex-shrink-0" />
              ...
              {loading ? (
                ...
                <div className="space-y-4">
                  ...
                  {activeTab === 'npcs' && (
                    <>
                      {npcs.length === 0 ? (
                        <p className="text-tavern-mauve text-center py-12">No saved NPCs yet</p>
                      ) : (
                        npcs.map((npc) => (
                          <div
                            key={npc.id}
                            ...
                            onClick={() => {
                              logger.debug('[SavedContent] Opening NPC detail:', npc)
                  ...
                  {activeTab === 'monsters' && (
                    <>
                      {monsters.length === 0 ? (
                        <p className="text-tavern-mauve text-center py-12">No saved monsters yet</p>
                      ) : (
                        monsters.map((monster) => (
                          <div
                            key={monster.id}
                            className="bg-background-panel border border-border rounded-lg p-4 hover:border-tavern-purple transition-colors cursor-pointer"
                            onClick={() => setSelectedItem({ ...monster, type: 'monsters' })}
                          >
                            <div className="flex justify-between items-start">
                              <div className="flex-1">
                                <h3 className="text-xl font-bold text-tavern-light mb-2">
                                ...
                                <p className="text-tavern-mauve">CR {monster.cr}</p>
                                <p className="text-sm text-text-muted mt-2">
                      ...
                      {encounters.length === 0 ? (
                        ...
                        encounters.map((encounter) => (
                          <div
                            key={encounter.id}
                            className="bg-background-panel border border-border rounded-lg p-4 hover:border-tavern-purple transition-colors cursor-pointer"
                            onClick={() => setSelectedItem({ ...encounter, type: 'encounters' })}
                          >
                            <div className="flex justify-between items-start">
                              <div className="flex-1">
                                <h3 className="text-xl font-bold text-tavern-light mb-2">
                                ...
                                <div className="flex gap-2 mb-2">
                                  ...
                                  <span className="text-tavern-mauve text-sm">
                                    Party: {encounter.party_size} (Lvl {encounter.party_level})
                                  </span>
                                </div>
                      ...
                      {dialogues.length === 0 ? (
                        ...
                        dialogues.map((dialogue) => (
                          <div
                            key={dialogue.id}
                            className="bg-background-panel border border-border rounded-lg p-4 hover:border-tavern-purple transition-colors cursor-pointer"
                            onClick={() => setSelectedItem({ ...dialogue, type: 'dialogues' })}
                          >
                            <div className="flex justify-between items-start">
                              <div className="flex-1">
                                <h3 className="text-xl font-bold text-tavern-light mb-2">
                                ...
                                {dialogue.mood && (
                                  <p className="text-tavern-mauve">Mood: {dialogue.mood}</p>
                                )}
                                <p className="text-sm text-text-muted mt-2">
                      ...
                      {locations.length === 0 ? (
                        ...
                        locations.map((location) => (
                          <div
                            key={location.id}
                            className="bg-background-panel border border-border rounded-lg p-4 hover:border-tavern-purple transition-colors cursor-pointer"
                            onClick={() => setSelectedItem({ ...location, type: 'locations' })}
                          >
                            <div className="flex justify-between items-start">
                              <div className="flex-1">
                                <h3 className="text-xl font-bold text-tavern-light mb-2">
                                ...
                                <p className="text-tavern-mauve capitalize">{location.type}</p>
                  ...
                  {activeTab === 'quests' && (
                    <>
                      {quests.length === 0 ? (
                        <p className="text-tavern-mauve text-center py-12">No saved quests yet</p>
                      ) : (
                        quests.map((quest) => (
                          <div
                            key={quest.id}
                            className="bg-background-panel border border-border rounded-lg p-4 hover:border-tavern-purple transition-colors cursor-pointer"
                            onClick={() => setSelectedItem({ ...quest, type: 'quests' })}
                          >
                            <div className="flex justify-between items-start">
                              <div className="flex-1">
                                <h3 className="text-xl font-bold text-tavern-light mb-2">
                  ...
                  {activeTab === 'items' && (
                    <>
                      {items.length === 0 ? (
                        <p className="text-tavern-mauve text-center py-12">No saved items yet</p>
                      ) : (
                        items.map((item) => (
                          <div
                            key={item.id}
                            className="bg-background-panel border border-border rounded-lg p-4 hover:border-tavern-purple transition-colors cursor-pointer"
                            onClick={() => setSelectedItem({ ...item, type: 'items' })}
                          >
                            <div className="flex justify-between items-start">
                              <div className="flex-1">
                                <h3 className="text-xl font-bold text-tavern-light mb-2">
                  ...
                  {activeTab === 'rumors' && (
                    <>
                      {rumors.length === 0 ? (
                        <p className="text-tavern-mauve text-center py-12">No saved rumors yet</p>
                      ) : (
                        rumors.map((rumor) => (
                          <div
                            key={rumor.id}
                            className="bg-background-panel border border-border rounded-lg p-4 hover:border-tavern-purple transition-colors cursor-pointer"
                            onClick={() => setSelectedItem({ ...rumor, type: 'rumors' })}
                          >
                            <div className="flex justify-between items-start">
                              <div className="flex-1">
                  ...
                  {activeTab === 'taverns' && (
                    <>
                      {taverns.length === 0 ? (
                        <p className="text-tavern-mauve text-center py-12">No saved taverns yet</p>
                      ) : (
                        taverns.map((tavern) => (
                          <div
                            key={tavern.id}
                            className="bg-background-panel border border-border rounded-lg p-4 hover:border-tavern-purple transition-colors cursor-pointer"
                            onClick={() => setSelectedItem({ ...tavern, type: 'taverns' })}
                          >
                            <div className="flex justify-between items-start">
                              <div className="flex-1">
                      ...
                      {merchants.length === 0 ? (
                        ...
                        merchants.map((merchant) => (
                          <div
                            key={merchant.id}
                            className="bg-background-panel border border-border rounded-lg p-4 hover:border-tavern-purple transition-colors cursor-pointer"
                            onClick={() => setSelectedItem({ ...merchant, type: 'merchants' })}
                          >
                            <div className="flex justify-between items-start">
                              <div className="flex-1">
                                <h3 className="text-lg font-semibold text-primary mb-1">
                  ...
                  {activeTab === 'traps' && (
                    <>
                      {traps.length === 0 ? (
                        <p className="text-tavern-mauve text-center py-12">No saved traps yet</p>
                      ) : (
                        traps.map((trap) => (
                          <div
                            key={trap.id}
                            className="bg-background-panel border border-border rounded-lg p-4 hover:border-tavern-purple transition-colors cursor-pointer"
                            onClick={() => setSelectedItem({ ...trap, type: 'traps' })}
                          >
                            <div className="flex justify-between items-start">
                              <div className="flex-1">
                                <h3 className="text-lg font-semibold text-primary mb-1">
                  ...
                  {activeTab === 'critters' && (
                    <>
                      {critters.length === 0 ? (
                        <p className="text-tavern-mauve text-center py-12">No saved critters yet</p>
                      ) : (
                        critters.map((critter) => (
                          <div
                            key={critter.id}
                            className="bg-background-panel border border-border rounded-lg p-4 hover:border-tavern-purple transition-colors cursor-pointer"
                            onClick={() => setSelectedItem({ ...critter, type: 'critters' })}
                          >
                            <div className="flex justify-between items-start">
                              <div className="flex-1">
                                <h3 className="text-lg font-semibold text-primary mb-1">
                  ...
                  {activeTab === 'chases' && (
                    <>
                      {chases.length === 0 ? (
                        <p className="text-tavern-mauve text-center py-12">No saved chases yet</p>
                      ) : (
                        chases.map((chase) => (
                          <div
                            key={chase.id}
                            className="bg-background-panel border border-border rounded-lg p-4 hover:border-tavern-purple transition-colors cursor-pointer"
                            onClick={() => setSelectedItem({ ...chase, type: 'chases' })}
                          >
                            <div className="flex justify-between items-start">
                              <div className="flex-1">
                                <h3 className="text-lg font-semibold text-primary mb-1">
