...
export default function LibrarySettings() {
  const { librarySettings, updateLibrarySettings, setLibraryContentTypeEnabled } =
    useUISettingsStore()

  const viewModes: { id: LibraryViewMode; label: string; icon: string; description: string }[] = [
    { id: 'list', label: 'List', icon: 'List', description: 'Detailed list view' },
    { id: 'grid', label: 'Grid', icon: 'Grid3X3', description: 'Card-based grid' },
    { id: 'compact', label: 'Compact', icon: 'AlignJustify', description: 'Dense list view' },
  ]
  ...
  return (
    <div className="space-y-8">
            ...
            >
              <Icon
                ...
                className={`w-6 h-6 mx-auto mb-2 ${
                  librarySettings.viewMode === mode.id ? 'text-primary' : 'text-text-muted'
                }`}
              />
      ...
      {/* Sort Options */}
      <div>
        <h3 className="text-lg font-semibold text-text mb-2">Default Sorting</h3>
        <p className="text-sm text-text-muted mb-4">
        ...
        <div className="flex gap-4">
          <div className="flex-1">
            ...
            <select
              ...
              onChange={(e) => updateLibrarySettings({ sortBy: e.target.value as LibrarySortBy })}
      ...
      <div>
        ...
        <p className="text-sm text-text-muted mb-4">How many items to show before pagination</p>
        <div className="flex gap-2">
      ...
      {/* Display Options */}
      <div>
        <h3 className="text-lg font-semibold text-text mb-2">Display Options</h3>
        <p className="text-sm text-text-muted mb-4">
        ...
        <div className="space-y-3">
          <label className="flex items-center justify-between p-3 rounded-lg border border-border bg-background hover:bg-background-panel transition-colors cursor-pointer">
            <div>
              ...
              <p className="text-sm text-text-muted">Display badge on AI-generated content</p>
            </div>
            <div
              className={`w-12 h-6 rounded-full p-1 transition-colors cursor-pointer ${
                librarySettings.showAIBadge ? 'bg-primary' : 'bg-border'
              }`}
              onClick={() => updateLibrarySettings({ showAIBadge: !librarySettings.showAIBadge })}
            >
              <div
                className={`w-4 h-4 rounded-full bg-white transition-transform ${
                  librarySettings.showAIBadge ? 'translate-x-6' : 'translate-x-0'
                }`}
              />
            </div>
          </label>
          ...
          <label className="flex items-center justify-between p-3 rounded-lg border border-border bg-background hover:bg-background-panel transition-colors cursor-pointer">
            <div>
              <span className="text-text font-medium">Show Campaign Filter</span>
              <p className="text-sm text-text-muted">Display campaign filter dropdown</p>
            </div>
            <div
              className={`w-12 h-6 rounded-full p-1 transition-colors cursor-pointer ${
              ...
              onClick={() =>
                updateLibrarySettings({ showCampaignFilter: !librarySettings.showCampaignFilter })
              }
            >
              <div
                className={`w-4 h-4 rounded-full bg-white transition-transform ${
                  librarySettings.showCampaignFilter ? 'translate-x-6' : 'translate-x-0'
                }`}
              />
            </div>
          </label>
      ...
      <div>
        <h3 className="text-lg font-semibold text-text mb-2">Visible Content Types</h3>
        <p className="text-sm text-text-muted mb-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {contentTypes.map((type) => {
            ...
            return (
              <button
                key={type.id}
                onClick={() => setLibraryContentTypeEnabled(type.id, !isEnabled)}
      ...
      <div>
        <h3 className="text-lg font-semibold text-text mb-2">Default Content Type</h3>
        <p className="text-sm text-text-muted mb-4">
        ...
        <select
          ...
          onChange={(e) =>
            updateLibrarySettings({ defaultContentType: e.target.value as ContentType })
          }
