...
export default function CombatTracker() {
  ...
  const handleCreateCombat = async (name: string, difficulty?: string, environment?: string) => {
    if (!sessionId) {
  ...
  const handleAddParticipant = async (participant: Omit<CombatParticipant, 'id' | 'combat_id'>) => {
    if (!combat) return
  ...
  // Update participant
  const handleUpdateParticipant = async (id: string, updates: Partial<CombatParticipant>) => {
    if (!combat) return

    try {
      const res = await authFetch(`/api/v1/combat/${combat.id}/participants/${id}`, {
        method: 'PUT',
    ...
    return (
      <div className="h-full flex items-center justify-center bg-background">
        <div className="text-center">
          <Icon name="AlertCircle" className="w-16 h-16 text-text-muted mx-auto mb-4" />
          <h2 className="text-xl font-bold text-text mb-2">No Campaign Selected</h2>
          <p className="text-text-muted">Select a campaign to start tracking combat.</p>
        </div>
      </div>
    )
  }
  ...
  if (showSetup || !combat) {
    return <CombatSetup onCreateCombat={handleCreateCombat} loading={loading} />
  }
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="flex items-center gap-4 mt-1 text-sm text-text-muted">
              ...
              {combat.environment && <span className="text-text-muted">{combat.environment}</span>}
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {currentParticipant ? (
            <ParticipantCard
              ...
              isCurrentTurn={true}
              onUpdate={(updates) => handleUpdateParticipant(currentParticipant.id, updates)}
              onRemove={() => handleRemoveParticipant(currentParticipant.id)}
            />
          ) : (
            <div className="text-center py-12">
              <Icon name="Users" className="w-16 h-16 text-text-muted mx-auto mb-4 opacity-50" />
              <h2 className="text-xl font-bold text-text mb-2">No Participants</h2>
              <p className="text-text-muted">Add combatants using the sidebar to begin.</p>
            </div>
          )}
          ...
          {participants.length > 0 && (
            <div className="mt-8">
              <h3 className="text-lg font-semibold text-text mb-4">All Combatants</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {participants.map((p, index) => (
                  <ParticipantCard
                    key={p.id}
                    participant={p}
                    isCurrentTurn={index === combat.current_turn % participants.length}
                    compact
                    onUpdate={(updates) => handleUpdateParticipant(p.id, updates)}
