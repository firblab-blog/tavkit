...
export default function ShoppingSession() {
  ...
  const handleCreateEncounter = async (data: {
    ...
    setIsLoading(true);
    try {
      const relationship = RELATIONSHIP_LEVELS.find(
        (r) => r.value === data.relationship_level,
      );
  ...
  const handleUpdateCartItem = async (
    ...
    try {
      const response = await fetch(
        `/api/v1/shopping/${encounter?.id}/cart/${itemId}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
  ...
  const handleUpdateHaggling = async (
    sessionId: string,
    updates: Partial<HagglingSession>,
  ) => {
    try {
      const response = await fetch(
        `/api/v1/shopping/${encounter?.id}/haggle/${sessionId}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
      ...
      if (response.ok) {
        ...
        if (updates.mood_change && encounter) {
          const newMood = Math.max(
            -5,
            Math.min(5, encounter.merchant_mood + updates.mood_change),
          );
  ...
  const handleEndSession = async () => {
    ...
    const total = purchasedItems.reduce((acc, item) => {
      const price = parseFloat(
        (item.negotiated_price || item.base_price).replace(/[^\d.]/g, ""),
      );
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-text-muted capitalize">
                {encounter.relationship_level} relationship •{" "}
                {encounter.discount_percentage}% discount
              </p>
            </div>
          </div>
