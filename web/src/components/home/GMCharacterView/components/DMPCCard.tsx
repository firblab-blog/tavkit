...
export default function DMPCCard({ campaignId }: DMPCCardProps) {
  ...
  const [campaignCharacterLinks, setCampaignCharacterLinks] = useState<
    CampaignCharacterLink[]
  >([]);
  ...
  useEffect(() => {
    if (campaignCharacterLinks.length > 0 && characters.length > 0) {
      ...
      const linkedCharacterIds = new Set(
        campaignCharacterLinks.map((link) => link.character_id),
      );
      const userCharacterInCampaign = characters.find((char) =>
        linkedCharacterIds.has(char.id),
      );
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-text-muted text-sm">
            Level {gmCharacter.level} {gmCharacter.race}{" "}
            {gmCharacter.class_info}
          </p>
          {/* Quick Stats */}
