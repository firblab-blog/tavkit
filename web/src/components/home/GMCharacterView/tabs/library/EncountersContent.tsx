      ...
      >
        <div className="space-y-3">
          {filteredItems.map((encounter) => {
            const diffColor = difficultyColors[encounter.difficulty] || difficultyColors.medium
            return (
              <ContentCard
                key={encounter.id}
                ...
                badges={[
                  ...
                  ...(encounter.xp_total
                    ? [{ label: `${encounter.xp_total} XP`, color: 'text-amber-400', bgColor: 'bg-amber-500/10' }]
                    : []),
                ]}
...
function EncounterDetailModal({ encounter, onClose, onDelete }: EncounterDetailModalProps) {
  const name = encounter.name || `${encounter.difficulty} Encounter`
  const diffColor = difficultyColors[encounter.difficulty] || difficultyColors.medium
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {encounter.xp_total && (
            <div className="px-4 py-2 bg-amber-500/10 border border-amber-500/30 rounded-lg">
              ...
              <p className="text-lg font-semibold text-amber-400">{encounter.xp_total}</p>
            </div>
          )}
          {encounter.xp_per_player && (
            <div className="px-4 py-2 bg-background border border-border rounded-lg">
              ...
              <p className="text-lg font-semibold text-text">{encounter.xp_per_player}</p>
            </div>
          )}
        </div>
            ...
            <div className="bg-background p-4 rounded-lg border border-border">
              {environment.terrain && (
                <p className="text-text">
                  <span className="text-text-muted">Terrain:</span> {environment.terrain}
                </p>
              )}
              {environment.lighting && (
                <p className="text-text">
                  <span className="text-text-muted">Lighting:</span> {environment.lighting}
                </p>
              )}
        ...
        {treasure && (
          <div>
            ...
            <div className="bg-background p-4 rounded-lg border border-border">
              {treasure.gold && (
                <p className="text-amber-400 font-semibold">{treasure.gold} gold pieces</p>
              )}
        ...
        {encounter.notes && (
          <div>
            ...
            <p className="text-text leading-relaxed whitespace-pre-wrap">{encounter.notes}</p>
          </div>
        )}
      </div>
    </ContentDetailModal>
