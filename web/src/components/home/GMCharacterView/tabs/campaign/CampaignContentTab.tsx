...
export default function CampaignContentTab({ campaignId }: CampaignContentTabProps) {
  const [searchParams, setSearchParams] = useSearchParams()

  // Get sub-tab from URL, default to 'overview'
  const subTabParam = searchParams.get('subtab')
  const activeSubTab: ContentSubTab = VALID_SUB_TABS.includes(subTabParam as ContentSubTab)
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Content */}
      {activeSubTab === 'overview' && <CampaignOverviewContent campaignId={campaignId} />}
      {activeSubTab === 'sessions' && <SessionsContent campaignId={campaignId} />}
      {activeSubTab === 'pcs' && <PlayerCharactersContent campaignId={campaignId} />}
      {activeSubTab === 'factions' && <FactionsContent campaignId={campaignId} />}
      {activeSubTab === 'lore' && <LoreContent campaignId={campaignId} />}
      {activeSubTab === 'locations' && <LocationsContent campaignId={campaignId} />}
      {activeSubTab === 'monsters' && <MonstersContent campaignId={campaignId} />}
      {activeSubTab === 'maps-art' && <MapsArtContent campaignId={campaignId} />}
      {activeSubTab === 'gm-notes' && <GMNotesContent campaignId={campaignId} />}
    </div>
  )
}
