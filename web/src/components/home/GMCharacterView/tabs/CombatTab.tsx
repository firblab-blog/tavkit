...
export default function CombatTab({ campaignId }: CombatTabProps) {
  ...
  const handleEndCombat = useCallback(async () => {
    if (!combat) return;
    if (!globalThis.confirm("Are you sure you want to end this combat?"))
      return;
  ...
  const handleUpdateHP = useCallback(
    async (participantId: string, delta: number) => {
      ...
      await updateParticipant(combat.id, participantId, {
        current_hp: newHP,
        temp_hp: newTempHP,
      });
    },
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-text-muted text-sm">
                When you start combat, players in your campaign will see the
                initiative order and can track their own HP and conditions in
                real-time.
              </p>
            </div>
          </div>
...
function ParticipantRow({
  ...
  const hpPercent =
    participant.max_hp > 0
      ? (participant.current_hp / participant.max_hp) * 100
      : 0;
