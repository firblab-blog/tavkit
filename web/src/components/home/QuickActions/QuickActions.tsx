...
export default function QuickActions() {
  ...
  const getToolsPath = () => {
    ...
    if (location.pathname.includes("/sandbox"))
      return "/dashboard/sandbox/tools";
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (editingTool) {
          // Update in list
          setCustomTools(
            customTools.map((t) => (t.id === editingTool.id ? savedTool : t)),
          );
        } else {
          // Add to list
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="bg-background border border-border/50 rounded-lg p-3">
                <p className="text-xs text-text-muted">
                  The tool will attempt to fetch the site's favicon
                  automatically. You can add popular D&D tools like 5e.tools,
                  koboldplus.club, donjon.bin.sh, or any other useful website.
                </p>
              </div>
