...
export default function CharacterEditForm({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      payload.current_hit_points = formData.current_hit_points || formData.max_hit_points || 0
      ...
      const response = await apiClient.put(`/characters/${character.id}`, payload)
      logger.debug('Character updated successfully:', response.data)
      onSuccess()
    } catch (err: any) {
      setError(err.response?.data?.error || err.message || 'Failed to update character')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
      ...
      {/* Ability Scores */}
      <CollapsibleSection title="Ability Scores" icon="Dices" defaultOpen={true}>
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => handleNumberChange('current_hit_points', e.target.value)}
              placeholder="—"
              className="w-full px-3 py-2 bg-background border border-border rounded-lg text-text placeholder-text-muted focus:outline-none focus:ring-2 focus:ring-primary"
              disabled={loading}
            />
