...
const iconColorClasses: Record<string, { bg: string; text: string; border: string }> = {
...
export default function ContentCard({
  ...
  return (
    ...
    >
      <div className={`flex ${isGrid ? 'flex-col' : 'items-start justify-between gap-3'}`}>
        <div className={`flex items-start gap-3 ${isGrid ? 'mb-3' : 'min-w-0 flex-1'}`}>
          {icon && (
            <div
          ...
          <div className="min-w-0 flex-1">
            ...
            {preview && <p className="text-text-muted text-sm mt-1 line-clamp-2">{preview}</p>}
            {formattedDate && <p className="text-text-muted text-xs mt-2">{formattedDate}</p>}
          </div>
        </div>

        {/* Actions */}
        {(onAssign || onDelete || actions) && (
          <div className={`flex gap-1 ${isGrid ? 'justify-end' : 'flex-shrink-0'}`}>
            {onAssign && (
              <button
