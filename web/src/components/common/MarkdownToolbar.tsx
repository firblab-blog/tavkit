...
export default function MarkdownToolbar({ textareaRef, value, onChange }: MarkdownToolbarProps) {
  ...
  const insertLinePrefix = (prefix: string) => {
    ...
    const newValue = value.substring(0, lineStart) + prefixedLines + value.substring(actualLineEnd)
    onChange(newValue)
  ...
  const insertNumberedList = () => {
    ...
    const newValue = value.substring(0, lineStart) + numberedLines + value.substring(actualLineEnd)
    onChange(newValue)
