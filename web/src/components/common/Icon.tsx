...
export default function Icon({ name, className = '', size }: IconProps) {
  ...
  // Lucide Icons
  if (iconSet === 'lucide') {
    const LucideIcon = LucideIcons[mapping.lucide as keyof typeof LucideIcons] as any
    if (!LucideIcon) {
  ...
  if (iconSet === 'heroicons') {
    ...
    return (
      <HeroIcon className={className} style={size ? { width: size, height: size } : undefined} />
    )
  }

  // React Icons (Game Icons)
  if (iconSet === 'react-icons') {
    const GameIcon = ReactIcons[mapping.reactIcons as keyof typeof ReactIcons] as any
    if (!GameIcon) {
  ...
  // Tabler Icons
  if (iconSet === 'tabler') {
    const TablerIcon = TablerIcons[mapping.tabler as keyof typeof TablerIcons] as any
    if (!TablerIcon) {
  ...
  // Phosphor Icons
  if (iconSet === 'phosphor') {
    const PhosphorIcon = PhosphorIcons[mapping.phosphor as keyof typeof PhosphorIcons] as any
    if (!PhosphorIcon) {
