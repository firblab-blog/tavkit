...
import { SearchResult, getResultTypeIcon, getResultTypeLabel } from '../../api/search'
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-border">
          ...
          />
          {loading && <Icon name="Loader2" className="w-5 h-5 text-primary animate-spin" />}
          <div className="flex items-center gap-1 text-xs text-text-muted">
            <kbd className="px-1.5 py-0.5 bg-tavern-dark rounded text-xs">esc</kbd>
            <span>to close</span>
          </div>
        </div>
        ...
        <div className="max-h-80 overflow-y-auto">
          {results.length === 0 && query.length >= 2 && !loading && (
            <div className="py-8 text-center text-text-muted">
              <Icon name="Search" className="w-10 h-10 mx-auto mb-2 opacity-50" />
          ...
          {results.length === 0 && query.length < 2 && (
            <div className="py-8 text-center text-text-muted">
              ...
              <div className="mt-4 flex flex-wrap justify-center gap-2">
                {['NPCs', 'Items', 'Locations', 'Quests', 'Characters'].map((type) => (
                  <span key={type} className="px-2 py-1 bg-tavern-dark rounded text-xs">
                    {type}
                  </span>
                ))}
              </div>
          ...
          {results.map((result, index) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-colors ${
                index === selectedIndex ? 'bg-primary/20' : 'hover:bg-tavern-dark/50'
              }`}
            >
              <div
              ...
              >
                <Icon name={getResultTypeIcon(result.type)} className="w-4 h-4 text-text" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-text font-medium truncate">{result.name}</div>
                {result.preview && (
                  <div className="text-sm text-text-muted truncate">{result.preview}</div>
                )}
              </div>
