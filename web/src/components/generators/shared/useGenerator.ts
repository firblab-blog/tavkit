...
import { AIGenerationSettings, getMaxTokensFromSettings } from '../AISettings'
import { emitContentSaved } from '@/lib/contentEvents'
import { getErrorMessage } from '@/api/generators'
import { EntryMode } from './EntryModeToggle'
...
export interface GeneratorConfig<TGenerated, TManual, TParams> {
  ...
  generateApi: (params: TParams, timeout: number) => Promise<TGenerated>
  /** Function to save content via API */
  saveApi: (data: unknown) => Promise<unknown>
  /** Function to normalize the API response */
  ...
  buildSavePayload: (data: TGenerated, campaignId: string | null) => Record<string, unknown>
  /** Function to build save payload from manual entry data */
  buildManualSavePayload: (data: TManual, campaignId: string | null) => Record<string, unknown>
...
export function useGenerator<TGenerated, TManual, TParams>(
  ...
  // Manual entry state
  const [manualData, setManualData] = useState<TManual>(config.defaultManualData)
  ...
  const generate = useCallback(
    async (params: Record<string, unknown>) => {
      ...
      try {
        ...
        if (rawEntity) {
          const normalized = config.normalizeResponse(rawEntity as Record<string, unknown>)
          ...
          const normalized = config.normalizeResponse(data as unknown as Record<string, unknown>)
          ;(normalized as Record<string, unknown>)._parseError =
