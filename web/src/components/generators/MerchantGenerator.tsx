  ...
  if (raw.description && typeof raw.description === "string") {
    ...
    if (descStr.startsWith("{") && descStr.endsWith("}")) {
      try {
        ...
        logger.debug(
          "[MerchantGenerator] Parsed merchant from JSON description:",
          parsedMerchant,
        );
...
export default function MerchantGenerator() {
  ...
  const [manualData, setManualData] =
    useState<ManualMerchantData>(defaultMerchantData);
  ...
  const generateMerchant = async () => {
    ...
    try {
      ...
      if (data.merchant) {
        ...
        const normalized = normalizeMerchantResponse(
  ...
  return (
    <>
      <GeneratorLayout
        ...
        formIcon="Settings"
        resultsTitle={
          entryMode === "manual" ? "Manual Entry" : "Generated Merchant"
        }
