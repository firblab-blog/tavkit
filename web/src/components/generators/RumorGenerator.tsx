...
export default function RumorGenerator() {
  ...
  const [manualData, setManualData] =
    useState<ManualRumorData>(defaultRumorData);
  ...
  const [rumorsResponse, setRumorsResponse] = useState<RumorsResponse | null>(
    null,
  );
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const normalized = normalizeRumorsResponse(
      ...
      if (!hasValidRumorContent(normalized)) {
        normalized._parseError =
          normalized._parseError ||
          "AI response missing essential rumor content.";
      }
  ...
  const aiFormContent = (
    <>
      ...
      <FormField
        label="Number of Rumors"
        description="How many rumors to generate"
      >
        <input
          type="number"
          min="1"
    ...
    rumorsResponse && rumorsResponse.rumors.length > 0 ? (
      <div className="space-y-6">
        ...
        {rumorsResponse._parseError && (
          <ParseWarning message={rumorsResponse._parseError} />
        )}
        ...
        <div>
          ...
          <p className="text-text-muted">
            {rumorsResponse.rumors.length} rumors created
          </p>
        </div>
