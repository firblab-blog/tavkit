  ...
  if (raw.description && typeof raw.description === "string") {
    ...
    if (descStr.startsWith("{") && descStr.endsWith("}")) {
      try {
        ...
        logger.debug(
          "[TavernGenerator] Parsed tavern from JSON description:",
          parsedTavern,
        );
...
export default function TavernGenerator() {
  ...
  const [manualData, setManualData] =
    useState<ManualTavernData>(defaultTavernData);
  ...
  const generateTavern = async () => {
    ...
    try {
      ...
      if (data.tavern) {
        ...
        const normalized = normalizeTavernResponse(
  ...
  return (
    <>
      <GeneratorLayout
        ...
        formIcon="Settings"
        resultsTitle={
          entryMode === "manual" ? "Manual Entry" : "Generated Tavern"
        }
