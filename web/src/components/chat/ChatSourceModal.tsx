...
const CONTENT_TYPE_CONFIG: {
  ...
  },
  {
    ...
    label: "Items",
    icon: "Package",
    description: "Equipment and loot",
  },
  {
    ...
    try {
      ...
      if (packsRes.ok) {
        ...
        const scrapedPacks = packs.filter(
          (p: SettingPack) =>
            p.scrape_status === "completed" && p.total_chunks > 0,
        );
          ...
          <div className="flex-1 min-h-0 overflow-y-auto p-4 space-y-2">
              ...
              {expandedSections.has("campaign_content") && (
                <div className="border-t border-border bg-background-panel/50 p-4">
                  <p className="text-xs text-tavern-mauve mb-4">
                    Include data from your campaign content in AI responses.
                    Disable sources you don&apos;t want the AI to reference.
                  </p>
                  ...
                  <label className="flex items-start gap-3 cursor-pointer">
                    <input
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          include_campaign_summary: e.target.checked,
                        })
                      }
                      className="mt-1 w-4 h-4 rounded border-border bg-background text-primary focus:ring-primary"
                    />
            ...
            <div className="border border-border rounded-lg overflow-hidden">
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className={`px-2 py-1 text-xs rounded transition-colors ${
                      enabledWikiSources.length ===
                        availableWikiSources.length &&
              ...
              {expandedSections.has("wiki_knowledge") && (
                <div className="border-t border-border bg-background-panel/50 p-4">
                  <p className="text-xs text-tavern-mauve mb-4">
                    Include official D&D setting lore from indexed wiki sources.
                    Disabling sources can significantly speed up responses.
                  </p>
                  ...
                  {loadingWikiSources ? (
                    ...
                    <p className="text-sm text-tavern-mauve text-center py-4">
                      No wiki sources have been indexed yet. Enable and index
                      wiki sources in Settings.
                    </p>
                  ) : (
                      ...
                      {availableWikiSources.map((source) => (
                        <label
                          ...
                          className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-colors hover:bg-tavern-dark/30 ${
                            !enabledWikiSources.includes(source.slug)
                              ? "opacity-50"
                              : ""
                          }`}
                        >
                    ...
                    enabledWikiSources.length > 0 && (
                      <div className="mt-4 pt-4 border-t border-border/50">
                        ...
                        <p className="text-xs text-tavern-mauve mb-2">
                          Number of wiki sections to include. Lower = faster,
                          higher = more context.
                        </p>
                        <input
                          type="range"
