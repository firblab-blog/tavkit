  ...
  return (
    <div className="bg-background-panel border border-border rounded-xl overflow-hidden">
      ...
      {showAddForm && !disabled && (
        <form onSubmit={handleAddRumor} className="p-4 border-b border-border bg-background/50">
      ...
      {rumors.length === 0 ? (
        <div className="p-6 text-center text-text-muted">
          <Icon name="MessageCircle" className="w-8 h-8 mx-auto mb-2 opacity-50" />
  ...
  return (
    <div className={`p-4 ${rumor.verified ? 'bg-emerald-500/5' : ''}`}>
      <div className="flex items-start gap-3">
        ...
        >
          {rumor.verified && <Icon name="Check" className="w-3 h-3 text-white" />}
        </button>
        ...
        <div className="flex-1 min-w-0">
          ...
          {rumor.source_patron && (
            <p className="text-xs text-text-muted mt-1">
              Source: <span className="text-primary">{rumor.source_patron}</span>
            </p>
          )}
          {rumor.notes && <p className="text-xs text-text-muted mt-1 italic">{rumor.notes}</p>}
        </div>
