...
export const SOCIAL_SKILLS = ['Persuasion', 'Deception', 'Intimidation', 'Insight', 'Performance']

export const ENCOUNTER_TYPES: { value: string; label: string; icon: IconName }[] = [
...
export default function SocialEncounters() {
  ...
  const handleAddCheck = async (checkData: {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const newMood = Math.max(-5, Math.min(5, encounter.current_mood + moodChange))
        const newSuccessCount = success ? encounter.success_count + 1 : encounter.success_count
        const newFailureCount = !success ? encounter.failure_count + 1 : encounter.failure_count
  ...
  const handleEndEncounter = async (outcome: 'success' | 'failure' | 'abandoned') => {
    if (!encounter) return
      ...
      <div className="h-full flex flex-col bg-background">
        ...
        <div className="flex-none border-b border-border bg-background-panel px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl font-bold text-text">Social Encounters</h1>
        ...
        <div className="flex-1 overflow-auto p-6">
          <div className="max-w-2xl mx-auto">
            <SocialSetup onStart={handleCreateEncounter} isLoading={isLoading} />
          </div>
        </div>
      </div>
    )
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6">
            <MoodTracker
              ...
              onMoodChange={(mood) => handleUpdateEncounter({ current_mood: mood })}
              disabled={isResolved}
            />
            ...
            <div className="bg-background-panel border border-border rounded-xl p-4">
              ...
              <textarea
                ...
                onChange={(e) => handleUpdateEncounter({ notes: e.target.value })}
