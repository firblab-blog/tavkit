    ...
    <div className="bg-background-panel border border-border rounded-xl overflow-hidden">
      <div className="p-6 border-b border-border">
        ...
        <p className="text-sm text-text-muted mt-1">
          Set up a social encounter to track negotiations, persuasion attempts, and other social
          challenges.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        {/* Encounter Name */}
        <div>
          <label className="block text-sm font-medium text-text mb-2">Encounter Name</label>
          <input
            type="text"
            value={name}
        ...
        {/* Encounter Type */}
        <div>
          <label className="block text-sm font-medium text-text mb-2">Encounter Type</label>
        ...
        {/* Goal */}
        <div>
          <label className="block text-sm font-medium text-text mb-2">Goal</label>
          <textarea
            value={goal}
        ...
        <div>
          <label className="block text-sm font-medium text-text mb-2">
            Starting NPC Mood:{' '}
            <span className={getMoodColor(startingMood)}>{getMoodLabel(startingMood)}</span>
          </label>
          <div className="flex items-center gap-4">
            <input
              ...
              className="flex-1"
            />
            <span className={`w-16 text-center font-bold text-lg ${getMoodColor(startingMood)}`}>
              {startingMood > 0 ? '+' : ''}
              {startingMood}
            </span>
          </div>
        ...
        <div>
          ...
          <p className="text-xs text-text-muted mt-1">
            The party needs {successThreshold} successful checks to achieve their goal. 3 failures
            or reaching -5 mood ends the encounter unfavorably.
          </p>
        </div>
