...
export default function ChaseSetup({ onStartChase, onCancel }: ChaseSetupProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || err.message || 'Failed to create chase')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="h-full overflow-auto">
      <div className="max-w-4xl mx-auto p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Chase Type & Terrain */}
          <div className="p-4 bg-background-panel rounded-lg border border-border">
            <h3 className="text-lg font-semibold text-text mb-4">Chase Configuration</h3>

            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label htmlFor="chaseType" className="block text-sm font-medium text-text mb-2">
                    Chase Type *
                  </label>
                  <select
                    id="chaseType"
                ...
                <div>
                  <label htmlFor="terrain" className="block text-sm font-medium text-text mb-2">
                    Terrain *
                  </label>
                  <select
                    id="terrain"
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setStartingDistance(parseInt(e.target.value))}
                  className="w-full"
                />
                <p className="text-xs text-text-muted mt-1">
                  Initial gap between pursuer and quarry (~{startingDistance * 30}-
                  {startingDistance * 60} feet)
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setCatchThreshold(parseInt(e.target.value))}
                    className="w-full"
                  />
                  <p className="text-xs text-text-muted mt-1">Distance to catch quarry</p>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setEscapeThreshold(parseInt(e.target.value))}
                    className="w-full"
                  />
                  <p className="text-xs text-text-muted mt-1">Distance for quarry to escape</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 bg-background-panel rounded-lg border border-border">
            <div className="flex items-center gap-3 mb-3">
              ...
              />
              <label htmlFor="hasTimeLimit" className="text-sm font-medium text-text">
                Enable Time Limit
              </label>
            </div>
            ...
            {hasTimeLimit && (
              <div>
                <label htmlFor="maxRounds" className="block text-sm font-medium text-text mb-2">
                  Maximum Rounds
                </label>
                <input
                  type="number"
                ...
                />
                <p className="text-xs text-text-muted mt-1">
                  Chase ends after this many rounds (gates close, reinforcements arrive, etc.)
                </p>
              </div>
            )}
          </div>
