...
interface ContentRevealsState {
  ...
  createReveal: (campaignId: string, reveal: CreateRevealRequest) => Promise<ContentReveal>
  updateReveal: (
    campaignId: string,
    id: string,
...
export const useContentRevealsStore = create<ContentRevealsState>((set, get) => ({
  gmReveals: [],
  playerReveals: [],
  loading: false,
  error: null,
  ...
  fetchGMReveals: async (campaignId: string) => {
    ...
    try {
      const response = await apiClient.get(`/campaigns/${campaignId}/reveals`)
  ...
  createReveal: async (campaignId: string, reveal: CreateRevealRequest) => {
    set({ loading: true, error: null })
    try {
      const response = await apiClient.post(`/campaigns/${campaignId}/reveals`, reveal)
  ...
  // GM: Update a reveal
  updateReveal: async (campaignId: string, id: string, updates: Partial<CreateRevealRequest>) => {
    set({ loading: true, error: null })
    try {
      ...
      const response = await apiClient.put(`/campaigns/${campaignId}/reveals/${id}`, updateData)
      const updatedReveal = response.data as ContentReveal

      set((state) => ({
        gmReveals: state.gmReveals.map((r) => (r.id === id ? updatedReveal : r)),
        loading: false,
      }))
  ...
  fetchPlayerReveals: async (campaignId: string) => {
    set({ loading: true, error: null })
    try {
      const response = await apiClient.get(`/player/campaigns/${campaignId}/revealed`)
      set({ playerReveals: response.data || [], loading: false })
    } catch (error: unknown) {
      const err = error as { response?: { data?: { error?: string } } }
      set({
        error: err.response?.data?.error || 'Failed to fetch revealed content',
        loading: false,
      })
    }
  },
  ...
  invalidateCache: () => set({ gmReveals: [], playerReveals: [], error: null }),
}))
...
storeEvents.on(CAMPAIGN_CHANGED, () => {
  ...
  logger.debug('[contentRevealsStore] Cache invalidated due to campaign change')
})
