...
interface PlayerQuestState {
  ...
  updateQuest: (id: string, quest: Partial<CreateQuestTrackingRequest>) => Promise<QuestTracking>
...
export const usePlayerQuestStore = create<PlayerQuestState>((set, get) => ({
  ...
  fetchQuests: async (campaignId?: string, status?: QuestStatus) => {
    ...
    try {
      ...
      const response = await apiClient.get(`/player/quests${queryString ? `?${queryString}` : ''}`)
  ...
  createQuest: async (quest: CreateQuestTrackingRequest) => {
    ...
    try {
      ...
      set((state) => ({
        quests: [...state.quests, newQuest].sort((a, b) => b.priority - a.priority),
        loading: false,
      }))
      return newQuest
    } catch (error: unknown) {
  ...
  updateQuest: async (id: string, quest: Partial<CreateQuestTrackingRequest>) => {
    set({ loading: true, error: null })
    try {
