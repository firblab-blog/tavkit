...
export const useAbilityTrackingStore = create<AbilityTrackingStoreState>(
  (set, get) => ({
    abilities: [],
    spellSlots: [],
    loading: false,
    ...
    fetchAbilities: async (characterId: string) => {
      ...
      try {
        const response = await apiClient.get(
          `/player/abilities/character/${characterId}`,
        );
        set({
      ...
      try {
        ...
        set((state) => ({
          abilities: state.abilities.map((a) =>
            a.id === abilityId ? response.data : a,
          ),
          loading: false,
        }));
      } catch (error: unknown) {
    ...
    useAbility: async (_characterId: string, abilityId: string) => {
      ...
      try {
        ...
        set((state) => ({
          abilities: state.abilities.map((a) =>
            a.id === abilityId
              ? { ...a, current_uses: ability.current_uses }
              : a,
          ),
        }));
      }
    ...
    resetAbility: async (_characterId: string, abilityId: string) => {
      ...
      set((state) => ({
        abilities: state.abilities.map((a) =>
          a.id === abilityId ? { ...a, current_uses: 0 } : a,
        ),
      }));
    ...
    useSpellSlot: async (characterId: string, level: number) => {
      ...
      try {
        await apiClient.post(
          `/player/abilities/${characterId}/spell-slot/${level}/use`,
        );
      } catch {
        // Revert
        set((state) => ({
    ...
    restoreSpellSlot: async (characterId: string, level: number) => {
      ...
      try {
        await apiClient.post(
          `/player/abilities/${characterId}/spell-slot/${level}/restore`,
        );
      } catch {
        // Revert
        set((state) => ({
    ...
    setSpellSlots: async (characterId: string, slots: SpellSlotConfig[]) => {
      ...
      try {
        await apiClient.put(`/player/abilities/${characterId}/spell-slots`, {
          spell_slots: slots,
        });
      } catch {
    ...
    shortRest: async (characterId: string) => {
      ...
      try {
        await apiClient.post(
          `/player/abilities/character/${characterId}/rest`,
          {
...
storeEvents.on(CAMPAIGN_CHANGED, () => {
  ...
  logger.debug(
    "[abilityTrackingStore] Cache invalidated due to campaign change",
  );
});
