...
export const useCampaignStore = create<CampaignState>()(
  subscribeWithSelector(
    persist(
      ...
      openCreateCampaignModal: () => set({ createCampaignModalOpen: true, editingCampaignId: null }),
      openEditCampaignModal: (campaignId) => set({ createCampaignModalOpen: true, editingCampaignId: campaignId }),
      ...
      fetchCampaigns: async (forceRefresh = false) => {
        ...
        try {
          ...
          const summaryPromises = campaigns.map(async (campaign: Campaign) => {
            try {
      ...
      addCampaign: async (campaignData) => {
        ...
        try {
          ...
          const campaignWithMembership = {
            ...campaign,
            membership_type: campaignData.role === 'player' ? 'player_local' : 'owner',
          }
      ...
      persistActiveCampaign: async (id) => {
        ...
        try {
          ...
          if (!response.ok) {
            logger.error('[campaignStore] Failed to persist campaign activation')
          }
        } catch (error) {
          logger.error('[campaignStore] Failed to persist campaign activation:', error)
      ...
      fetchRecentActivity: async (campaignId) => {
        ...
        if (
          ...
          logger.debug('[campaignStore] fetchRecentActivity using cached data')
          return
        }
        ...
        try {
          ...
          const response = await authFetch(`${API_URL}/api/v1/campaigns/${campaignId}/activity`)
      ...
      generateInviteCode: async (campaignId, options = {}) => {
        try {
          const response = await authFetch(`${API_URL}/api/v1/campaigns/${campaignId}/invites`, {
            method: 'POST',
            body: JSON.stringify({
      ...
      joinCampaign: async (code, characterId) => {
        try {
          const response = await authFetch(`${API_URL}/api/v1/campaigns/join`, {
            method: 'POST',
            body: JSON.stringify({
              code,
      ...
      leaveCampaign: async (campaignId) => {
        try {
          const response = await authFetch(`${API_URL}/api/v1/campaigns/${campaignId}/leave`, {
            method: 'DELETE',
          })
      ...
      listCampaignMembers: async (campaignId) => {
        try {
          const response = await authFetch(`${API_URL}/api/v1/campaigns/${campaignId}/members`)
...
useCampaignStore.subscribe(
  ...
  (newId, prevId) => {
    if (newId !== prevId && prevId !== null) {
      ...
      logger.debug('[campaignStore] Campaign changed, emitted CAMPAIGN_CHANGED event', {
        from: prevId,
        to: newId,
      })
    }
...
import('./contextStore').then(({ useContextStore }) => {
  ...
  useContextStore.subscribe((state) => {
    ...
    if (contextCampaignId && contextCampaignId !== currentCampaignId) {
      logger.debug('[campaignStore] Syncing activeCampaignId from contextStore', {
