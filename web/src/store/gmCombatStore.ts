...
export const useGMCombatStore = create<GMCombatStoreState>((set, get) => ({
  ...
  fetchActiveCombat: async (campaignId: string) => {
    ...
    try {
      const response = await apiClient.get(
        `/campaigns/${campaignId}/combat/active`,
      );
  ...
  endCombat: async (combatId: string) => {
    set({ loading: true, error: null });
    try {
      await apiClient.put(`/combat/${combatId}`, {
        status: "completed",
        is_active: false,
      });
      set({ combat: null, participants: [], loading: false });
  ...
  updateParticipant: async (
    ...
    try {
      ...
      const err = error as { response?: { data?: { error?: string } } };
      set({
        error: err.response?.data?.error || "Failed to update participant",
      });
    }
  },
  ...
  removeParticipant: async (combatId: string, participantId: string) => {
    try {
      ...
      const err = error as { response?: { data?: { error?: string } } };
      set({
        error: err.response?.data?.error || "Failed to remove participant",
      });
    }
  },
  ...
  fetchSettings: async (campaignId: string) => {
    try {
      const response = await apiClient.get(
        `/campaigns/${campaignId}/combat-settings`,
      );
  ...
  updateSettings: async (
    campaignId: string,
    updates: Partial<CombatSettings>,
  ) => {
    try {
...
function handleWSMessage(
  ...
  switch (data.type) {
    ...
    case "combat:participant_updated": {
      const payload = data.payload as Partial<CombatParticipant> & {
        participant_id: string;
      };
      set({
        participants: get().participants.map((p) =>
