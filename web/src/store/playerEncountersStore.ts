...
interface PlayerEncountersState {
  ...
  updateNPC: (
    id: string,
    npc: Partial<CreateNPCEncounterRequest>,
  ) => Promise<NPCEncounter>;
  deleteNPC: (id: string) => Promise<void>;

  // Location Actions
  fetchLocations: (campaignId?: string) => Promise<void>;
  createLocation: (
    location: CreateLocationVisitRequest,
...
export const usePlayerEncountersStore = create<PlayerEncountersState>(
  (set, get) => ({
    npcs: [],
    locations: [],
    loadingNPCs: false,
    ...
    createNPC: async (npc: CreateNPCEncounterRequest) => {
      ...
      try {
        ...
        set((state) => ({
          npcs: [...state.npcs, newNPC].sort((a, b) =>
    ...
    updateNPC: async (id: string, npc: Partial<CreateNPCEncounterRequest>) => {
      ...
      try {
        ...
        const updateData = {
          ...
          first_met_session:
            npc.first_met_session ?? existing.first_met_session,
          first_met_location:
            npc.first_met_location ?? existing.first_met_location,
    ...
    fetchLocations: async (campaignId?: string) => {
      ...
      try {
        ...
        const response = await apiClient.get(
          `/player/locations-visited${params}`,
        );
        ...
        set({
          error:
            err.response?.data?.error || "Failed to fetch locations visited",
          loadingLocations: false,
        });
      }
    },
    ...
    createLocation: async (location: CreateLocationVisitRequest) => {
      ...
      try {
        const response = await apiClient.post(
          "/player/locations-visited",
          location,
        );
        const newLocation = response.data as LocationVisit;
        set((state) => ({
          locations: [...state.locations, newLocation].sort((a, b) =>
            a.name.localeCompare(b.name),
          ),
          loadingLocations: false,
        }));
    ...
    updateLocation: async (
      ...
      try {
        ...
        const updateData = {
          ...
          first_visit_session:
            location.first_visit_session ?? existing.first_visit_session,
          notes: location.notes ?? existing.notes,
        };

        const response = await apiClient.put(
          `/player/locations-visited/${id}`,
          updateData,
        );
...
storeEvents.on(CAMPAIGN_CHANGED, () => {
  ...
  logger.debug(
    "[playerEncountersStore] Cache invalidated due to campaign change",
  );
});
