...
export const usePlayerCombatStore = create<PlayerCombatStoreState>(
  (set, get) => ({
    ...
    fetchCombatState: async (characterId: string) => {
      ...
      try {
        const response = await apiClient.get(
          `/player/combat?character_id=${characterId}`,
        );
        set({
    ...
    endCombat: async (characterId: string) => {
      set((state) => ({
        combat: {
          ...
          reaction_used: false,
          conditions: state.combat.conditions.filter(
            (c) => c.type !== "concentrating",
          ),
        },
      }));
      try {
        ...
        combat: {
          ...
          concentration_spell:
            conditionType === "concentrating"
              ? undefined
              : state.combat.concentration_spell,
        },
      }));
    ...
    invalidateCache: () =>
      set({ combat: DEFAULT_COMBAT_STATE, error: null, lastSync: null }),
...
> = {
  ...
  grappled: {
    name: "Grappled",
    description:
      "Speed becomes 0. Ends if grappler is incapacitated or moved out of reach.",
  },
  incapacitated: {
    name: "Incapacitated",
  ...
  exhaustion: {
    name: "Exhaustion",
    description:
      "Cumulative levels causing increasing penalties. 6 levels = death.",
  },
  concentrating: {
    name: "Concentrating",
    description:
      "Maintaining a spell. Con save on damage, DC 10 or half damage taken.",
  },
};
