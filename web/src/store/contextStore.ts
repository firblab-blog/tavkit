...
interface ContextState {
  ...
  updateContext: (
    updates: Partial<
      Pick<
        UserContext,
        'last_context_type' | 'last_campaign_id' | 'last_character_id' | 'default_game_system'
      >
    >
  ) => Promise<void>
  ...
  updateContextSync: (
    updates: Partial<
      Pick<UserContext, 'last_context_type' | 'last_campaign_id' | 'last_character_id'>
    >
  ) => void
  ...
  persist(
    (set, get) => ({
      ...
      fetchContext: async () => {
        ...
        try {
          const response = await authFetch(`${API_URL}/api/v1/users/me/context`)
      ...
      updateContext: async (updates) => {
        ...
        try {
          ...
          const errorMessage = error instanceof Error ? error.message : 'Unknown error'
          logger.error('[contextStore] Failed to update context:', errorMessage)
          set({ error: errorMessage, loading: false })
          throw error
        }
      },
      ...
      updateContextSync: (updates) => {
        set((state) => ({
          userContext: state.userContext ? { ...state.userContext, ...updates } : null,
        }))
      },
      ...
      persistContext: async (updates, maxRetries = 3) => {
        for (let attempt = 0; attempt < maxRetries; attempt++) {
          try {
            ...
            logger.warn(`[contextStore] Persist attempt ${attempt + 1} failed with status ${response.status}`)
          } catch (error) {
            logger.warn(`[contextStore] Persist attempt ${attempt + 1} failed:`, error)
          }
        ...
        logger.error('[contextStore] Failed to persist context after all retries')
      ...
      completeOnboarding: async () => {
        set({ loading: true, error: null })
        try {
          const response = await authFetch(`${API_URL}/api/v1/users/me/onboarding`, {
            method: 'POST',
          })
          ...
          if (currentContext) {
            set({
              userContext: { ...currentContext, has_completed_onboarding: true },
              loading: false,
            })
          } else {
          ...
          const errorMessage = error instanceof Error ? error.message : 'Unknown error'
          logger.error('[contextStore] Failed to complete onboarding:', errorMessage)
          set({ error: errorMessage, loading: false })
          throw error
        }
      },
