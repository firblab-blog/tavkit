...
import { Theme, getTheme, getSemanticColors, defaultTheme, defaultMode } from '../config/themes'
...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  const [mode, setModeState] = useState<'light' | 'dark'>(() => {
    ...
    try {
      ...
      logger.error('[ThemeProvider] Error loading mode from localStorage:', error)
    }
  ...
  useEffect(() => {
    const loadThemeFromBackend = async () => {
      if (!isAuthenticated) {
        logger.debug('[ThemeProvider] Not authenticated, skipping backend load')
        return
      }

      logger.debug('[ThemeProvider] Authenticated, loading theme from backend...')

      try {
        // Settings is a public endpoint, but we use authFetch for consistency
        const response = await authFetch('/api/v1/settings')

        logger.debug('[ThemeProvider] Backend response status:', response.status)
        ...
        if (response.ok) {
          ...
          if (data.ui_settings) {
            const uiSettings =
              typeof data.ui_settings === 'string' ? JSON.parse(data.ui_settings) : data.ui_settings

            logger.debug('[ThemeProvider] Parsed ui_settings:', uiSettings)

            if (uiSettings.theme) {
              logger.debug('[ThemeProvider] Loaded theme from backend:', uiSettings.theme)
            ...
            if (uiSettings.mode) {
              logger.debug('[ThemeProvider] Loaded mode from backend:', uiSettings.mode)
        ...
        logger.error('[ThemeProvider] Error loading theme from backend:', error)
      }
    }
  ...
  useEffect(() => {
    ...
    root.style.setProperty('--color-primary-light', semanticColors.primaryLight)
    ...
    if (isAuthenticated) {
      try {
        ...
        if (getResponse.ok) {
          ...
          logger.debug('[ThemeProvider] Updated ui_settings:', JSON.stringify(uiSettings))
          ...
          logger.debug('[ThemeProvider] Sending PUT request with payload:', JSON.stringify(payload))
          ...
          logger.debug('[ThemeProvider] Save response status:', saveResponse.status)

          if (saveResponse.ok) {
            const responseData = await saveResponse.json()
            logger.debug('[ThemeProvider] Theme saved to backend successfully:', responseData)
          } else {
  ...
  return (
    <ThemeContext.Provider value={{ currentTheme, themeId, mode, setTheme, setMode }}>
      {children}
