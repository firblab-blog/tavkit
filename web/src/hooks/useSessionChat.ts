...
export function useSessionChat(options: UseSessionChatOptions = {}) {
  ...
  const [sourcePreferences, setSourcePreferences] = useState<ChatSourcePreferences | null>(null)
  ...
  useEffect(() => {
    ...
    const loadConversations = async () => {
      ...
      try {
        const response = await authFetch(`/api/v1/chat/conversations/${activeCampaign.id}`)
        if (response.ok) {
  ...
  useEffect(() => {
    ...
    const loadPreferences = async () => {
      ...
      try {
        const response = await authFetch(`/api/v1/chat/preferences/${activeCampaign.id}`)
        if (response.ok) {
          const data = await response.json()
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      if (!activeConversationId && data.conversation_id) {
        ...
        const convResponse = await authFetch(`/api/v1/chat/conversations/${activeCampaign.id}`)
        if (convResponse.ok) {
      ...
      const assistantMessage: ChatMessage = {
        ...
        campaign_id: activeCampaign.id,
        user_id: '',
        conversation_id: data.conversation_id || activeConversationId || undefined,
        role: 'assistant',
  ...
  const deleteConversation = useCallback(
    async (id: string, skipConfirm = false) => {
      if (!skipConfirm && !confirm('Delete this conversation? This cannot be undone.')) return
