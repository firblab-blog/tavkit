...
export function useLibraryContent<T extends { id: string }>(
  ...
  const { contentType, campaignId, showCampaignFilter = true, searchFields = [] } = options
  ...
  const fetchContent = useCallback(async () => {
    ...
    try {
      ...
      const message = err.response?.data?.error || err.message || `Failed to load ${contentType}`
      setError(message)
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => {
      ...
      if (anyItem.name && anyItem.name.toLowerCase().includes(query)) return true
      if (anyItem.title && anyItem.title.toLowerCase().includes(query)) return true
      return false
    })
  ...
  const deleteItem = useCallback(
    async (id: string) => {
      try {
        ...
        setError(message)
        logger.error(`[useLibraryContent] Failed to delete ${contentType}:`, err)
        throw err
      }
    },
