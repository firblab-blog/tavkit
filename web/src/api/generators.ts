...
export async function generateNPC(
  ...
  const response = await apiClient.post<NPCGenerationResponse>(
    "/npcs/generate",
    request,
    config,
  );
...
export async function generateItem(
  ...
  const config: AxiosRequestConfig = timeout ? { timeout: timeout * 1000 } : {};
  const response = await apiClient.post<ItemGenerationResponse>(
    "/items/generate",
    request,
    config,
  );
...
export async function saveTavern(
  request: Record<string, unknown>,
): Promise<{ id: string }> {
...
export async function saveRumor(
  request: Record<string, unknown>,
): Promise<{ id: string }> {
...
export async function generateTrap(
  ...
  const config: AxiosRequestConfig = timeout ? { timeout: timeout * 1000 } : {};
  const response = await apiClient.post<TrapGenerationResponse>(
    "/traps/generate",
    request,
    config,
  );
...
export async function saveTrap(
  request: Record<string, unknown>,
): Promise<{ id: string }> {
...
export async function saveCritter(
  request: Record<string, unknown>,
): Promise<{ id: string }> {
...
export async function saveChase(
  request: Record<string, unknown>,
): Promise<{ id: string }> {
