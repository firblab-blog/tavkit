...
export async function getCampaignItems(campaignId: string): Promise<ItemWithCampaignLink[]> {
...
export async function linkItemToCampaign(
  ...
  await apiClient.post(`/campaigns/${campaignId}/items/${itemId}`, options || {})
}

export async function unlinkItemFromCampaign(campaignId: string, itemId: string): Promise<void> {
