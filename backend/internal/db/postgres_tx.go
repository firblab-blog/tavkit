...
// PostgresTx implements Transaction interface for PostgreSQL
...
func (t *PostgresTx) CreateCampaignSummary(ctx context.Context, summary *CampaignSummary) error {
	if summary.ID == "" {
...
func (t *PostgresTx) GetCampaignSummaryByCampaignID(ctx context.Context, campaignID string) (*CampaignSummary, error) {
	...
	err := t.tx.QueryRow(ctx, query, campaignID).Scan(
		&summary.ID, &summary.CampaignID, &summary.UserID, &summary.Overview, &summary.SettingSummary,
...
func (t *PostgresTx) UpdateCampaignSummary(ctx context.Context, summary *CampaignSummary) error {
	summary.UpdatedAt = time.Now()
	summary.Version++
...
// Uses atomic ON CONFLICT to avoid race conditions
func (t *PostgresTx) UpsertCampaignContentStatus(ctx context.Context, status *CampaignContentStatus) error {
	if status.ID == "" {
	...
	// Use PostgreSQL's ON CONFLICT for atomic upsert within the transaction
...
func (t *PostgresTx) GetCampaignContentStatus(ctx context.Context, campaignID string, contentType string, contentID string) (*CampaignContentStatus, error) {
	...
	var statusData []byte
	err := t.tx.QueryRow(ctx, query, campaignID, contentType, contentID).Scan(
		&status.ID, &status.CampaignID, &status.ContentType, &status.ContentID,
		&status.Defeated, &status.Visited, &status.Obtained, &status.Heard,
		&status.Triggered, &status.Encountered, &status.Completed,
...
func (t *PostgresTx) UpdateCombatParticipant(ctx context.Context, participant *CombatParticipant) error {
...
func (t *PostgresTx) CreateCombatCondition(ctx context.Context, condition *CombatCondition) error {
	if condition.ID == "" {
...
func (t *PostgresTx) DeleteCombatCondition(ctx context.Context, id string) error {
...
func (t *PostgresTx) UpdateCombatEncounter(ctx context.Context, combat *CombatEncounter) error {
	query := `UPDATE combat_encounters
		  SET name = $1, current_round = $2, current_turn = $3, status = $4,
		      environment = $5, notes = $6
		  WHERE id = $7`
...
func (t *PostgresTx) CreateSessionEvent(ctx context.Context, event *SessionEvent) error {
	if event.ID == "" {
...
func (t *PostgresTx) UpdateCharacter(ctx context.Context, character *Character) error {
	character.UpdatedAt = time.Now()

	query := `UPDATE characters SET
		name = $1, level = $2, race = $3, subrace = $4, class_info = $5, subclass = $6,
		background = $7, alignment = $8, experience_points = $9, inspiration = $10,
		strength = $11, dexterity = $12, constitution = $13, intelligence = $14,
		wisdom = $15, charisma = $16, armor_class = $17, initiative = $18, speed = $19,
		max_hit_points = $20, current_hit_points = $21, temp_hit_points = $22,
		proficiency_bonus = $23, campaign_id = $24, updated_at = $25
		WHERE id = $26`
...
func (t *PostgresTx) LinkCharacterToCampaign(ctx context.Context, campaignID, characterID string) error {
	id := generateUUID()
	...
	_, err := t.tx.Exec(ctx, query, id, campaignID, characterID, time.Now())
	return err
}

// UnlinkCharacterFromCampaign unlinks a character from a campaign within the transaction
func (t *PostgresTx) UnlinkCharacterFromCampaign(ctx context.Context, campaignID, characterID string) error {
	query := `DELETE FROM campaign_characters WHERE campaign_id = $1 AND character_id = $2`
	_, err := t.tx.Exec(ctx, query, campaignID, characterID)
	return err
}
