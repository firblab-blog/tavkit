...
// extractFactsBatch extracts facts for a batch of content items
func (p *ChunkedSummaryPipeline) extractFactsBatch(ctx context.Context, campaignID, userID, contentType string, offset, limit int) error {
	// Get content items for this batch
	items, err := p.getContentBatch(ctx, campaignID, userID, contentType, offset, limit)
	if err != nil {
		return err
	}
